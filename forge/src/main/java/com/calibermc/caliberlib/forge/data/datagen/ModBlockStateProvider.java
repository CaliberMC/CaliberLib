...
public class ModBlockStateProvider extends BlockStateProvider {
    ...
    protected void registerStatesAndModels() {
        for (BlockManager blockManager : BlockManager.BLOCK_MANAGERS.get(this.modid)) {
            ...
            if (base != null && !base.skipRegister) {
                ModBlockStateProvider.accept(base, blockManager::mainChild, Pair.of(blockManager, this));
            }
            for (Map.Entry<BlockManager.BlockAdditional, Pair<ResourceLocation, Supplier<Block>>> e : blockManager.getBlocks().entrySet()) {
                if (e.getKey().variant != ModBlockFamily.Variant.BASE) {
                    try {
                        ModBlockStateProvider.accept(e.getKey(), e.getValue().getSecond(), Pair.of(blockManager, this));
                    } catch (Exception ex) {
                        ex.printStackTrace();
                    }
                }
    ...
    public static void accept(BlockManager.BlockAdditional block, Supplier<Block> sup, Pair<BlockManager, ModBlockStateProvider> pair) {
        var additional = BlockManager.DATA_GEN_MAP.get(block);
        AdditionalDataGenForge forge = new AdditionalDataGenForge(block);
        additional.accept(forge);
        forge.stateGenerator.accept(sup, pair);
    }
    ...
    public void archBlock(ArchBlock block, ResourceLocation side, ResourceLocation bottom, ResourceLocation top) {
        ModelFile arch = models().withExistingParent(name(block), modLoc("block/templates/arch"))
                ...
                arch_end = models().withExistingParent(name(block) + "_end", modLoc("block/templates/arch_end"))
                        .texture("side", side)
                        .texture("top", top)
                        .texture("bottom", bottom),
                arch_connected = models().withExistingParent(name(block) + "_connected", modLoc("block/templates/arch_connected"))
                        .texture("side", side)
                        .texture("top", top)
                        .texture("bottom", bottom),
        ...
        getVariantBuilder(block)
                .partialState().with(ArchBlock.FACING, Direction.NORTH).with(ArchBlock.TYPE, IntersectionShape.SINGLE)
                .modelForState().modelFile(arch).addModel()
                .partialState().with(ArchBlock.FACING, Direction.NORTH).with(ArchBlock.TYPE, IntersectionShape.END)
                .modelForState().modelFile(arch_end).addModel()
                .partialState().with(ArchBlock.FACING, Direction.NORTH).with(ArchBlock.TYPE, IntersectionShape.CONNECTED)
                .modelForState().modelFile(arch_connected).addModel()
                .partialState().with(ArchBlock.FACING, Direction.NORTH).with(ArchBlock.TYPE, IntersectionShape.INNER_LEFT)
                .modelForState().modelFile(arch_corner).addModel()
                .partialState().with(ArchBlock.FACING, Direction.NORTH).with(ArchBlock.TYPE, IntersectionShape.INNER_RIGHT)
                .modelForState().modelFile(arch_corner).rotationY(270).uvLock(true).addModel()
                .partialState().with(ArchBlock.FACING, Direction.NORTH).with(ArchBlock.TYPE, IntersectionShape.OUTER_LEFT)
                .modelForState().modelFile(arch_corner).rotationY(90).uvLock(true).addModel()
                .partialState().with(ArchBlock.FACING, Direction.NORTH).with(ArchBlock.TYPE, IntersectionShape.OUTER_RIGHT)
                .modelForState().modelFile(arch_corner).rotationY(180).uvLock(true).addModel()
                .partialState().with(ArchBlock.FACING, Direction.NORTH).with(ArchBlock.TYPE, IntersectionShape.CROSS)
                .modelForState().modelFile(arch_cross).addModel()
                .partialState().with(ArchBlock.FACING, Direction.NORTH).with(ArchBlock.TYPE, IntersectionShape.T)
                .modelForState().modelFile(arch_t).addModel()
                .partialState().with(ArchBlock.FACING, Direction.NORTH).with(ArchBlock.TYPE, IntersectionShape.T_LEFT)
                .modelForState().modelFile(arch_t).rotationY(90).uvLock(true).addModel()
                .partialState().with(ArchBlock.FACING, Direction.NORTH).with(ArchBlock.TYPE, IntersectionShape.T_RIGHT)
                .modelForState().modelFile(arch_t).rotationY(270).uvLock(true).addModel()
                .partialState().with(ArchBlock.FACING, Direction.NORTH).with(ArchBlock.TYPE, IntersectionShape.T_OPPOSITE)
                .modelForState().modelFile(arch_t).rotationY(180).uvLock(true).addModel()

                .partialState().with(ArchBlock.FACING, Direction.EAST).with(ArchBlock.TYPE, IntersectionShape.SINGLE)
                .modelForState().modelFile(arch).rotationY(90).uvLock(true).addModel()
                .partialState().with(ArchBlock.FACING, Direction.EAST).with(ArchBlock.TYPE, IntersectionShape.END)
                .modelForState().modelFile(arch_end).rotationY(90).uvLock(true).addModel()
                .partialState().with(ArchBlock.FACING, Direction.EAST).with(ArchBlock.TYPE, IntersectionShape.CONNECTED)
                .modelForState().modelFile(arch_connected).rotationY(90).uvLock(true).addModel()
                .partialState().with(ArchBlock.FACING, Direction.EAST).with(ArchBlock.TYPE, IntersectionShape.INNER_LEFT)
                .modelForState().modelFile(arch_corner).rotationY(90).uvLock(true).addModel()
                .partialState().with(ArchBlock.FACING, Direction.EAST).with(ArchBlock.TYPE, IntersectionShape.INNER_RIGHT)
                .modelForState().modelFile(arch_corner).addModel()
                .partialState().with(ArchBlock.FACING, Direction.EAST).with(ArchBlock.TYPE, IntersectionShape.OUTER_LEFT)
                .modelForState().modelFile(arch_corner).rotationY(180).uvLock(true).addModel()
                .partialState().with(ArchBlock.FACING, Direction.EAST).with(ArchBlock.TYPE, IntersectionShape.OUTER_RIGHT)
                .modelForState().modelFile(arch_corner).rotationY(270).uvLock(true).addModel()
                .partialState().with(ArchBlock.FACING, Direction.EAST).with(ArchBlock.TYPE, IntersectionShape.CROSS)
                .modelForState().modelFile(arch_cross).addModel()
                .partialState().with(ArchBlock.FACING, Direction.EAST).with(ArchBlock.TYPE, IntersectionShape.T)
                .modelForState().modelFile(arch_t).rotationY(90).uvLock(true).addModel()
                .partialState().with(ArchBlock.FACING, Direction.EAST).with(ArchBlock.TYPE, IntersectionShape.T_LEFT)
                .modelForState().modelFile(arch_t).rotationY(180).uvLock(true).addModel()
                .partialState().with(ArchBlock.FACING, Direction.EAST).with(ArchBlock.TYPE, IntersectionShape.T_RIGHT)
                .modelForState().modelFile(arch_t).addModel()
                .partialState().with(ArchBlock.FACING, Direction.EAST).with(ArchBlock.TYPE, IntersectionShape.T_OPPOSITE)
                .modelForState().modelFile(arch_t).rotationY(270).uvLock(true).addModel()
                
                .partialState().with(ArchBlock.FACING, Direction.SOUTH).with(ArchBlock.TYPE, IntersectionShape.SINGLE)
                .modelForState().modelFile(arch).rotationY(180).uvLock(true).addModel()
                .partialState().with(ArchBlock.FACING, Direction.SOUTH).with(ArchBlock.TYPE, IntersectionShape.END)
                .modelForState().modelFile(arch_end).rotationY(180).uvLock(true).addModel()
                .partialState().with(ArchBlock.FACING, Direction.SOUTH).with(ArchBlock.TYPE, IntersectionShape.CONNECTED)
                .modelForState().modelFile(arch_connected).rotationY(180).uvLock(true).addModel()
                .partialState().with(ArchBlock.FACING, Direction.SOUTH).with(ArchBlock.TYPE, IntersectionShape.INNER_LEFT)
                .modelForState().modelFile(arch_corner).rotationY(180).uvLock(true).addModel()
                .partialState().with(ArchBlock.FACING, Direction.SOUTH).with(ArchBlock.TYPE, IntersectionShape.INNER_RIGHT)
                .modelForState().modelFile(arch_corner).rotationY(90).uvLock(true).addModel()
                .partialState().with(ArchBlock.FACING, Direction.SOUTH).with(ArchBlock.TYPE, IntersectionShape.OUTER_LEFT)
                .modelForState().modelFile(arch_corner).rotationY(270).uvLock(true).addModel()
                .partialState().with(ArchBlock.FACING, Direction.SOUTH).with(ArchBlock.TYPE, IntersectionShape.OUTER_RIGHT)
                .modelForState().modelFile(arch_corner).addModel()
                .partialState().with(ArchBlock.FACING, Direction.SOUTH).with(ArchBlock.TYPE, IntersectionShape.CROSS)
                .modelForState().modelFile(arch_cross).addModel()
                .partialState().with(ArchBlock.FACING, Direction.SOUTH).with(ArchBlock.TYPE, IntersectionShape.T)
                .modelForState().modelFile(arch_t).rotationY(180).uvLock(true).addModel()
                .partialState().with(ArchBlock.FACING, Direction.SOUTH).with(ArchBlock.TYPE, IntersectionShape.T_LEFT)
                .modelForState().modelFile(arch_t).rotationY(270).uvLock(true).addModel()
                .partialState().with(ArchBlock.FACING, Direction.SOUTH).with(ArchBlock.TYPE, IntersectionShape.T_RIGHT)
                .modelForState().modelFile(arch_t).rotationY(90).uvLock(true).addModel()
                .partialState().with(ArchBlock.FACING, Direction.SOUTH).with(ArchBlock.TYPE, IntersectionShape.T_OPPOSITE)
                .modelForState().modelFile(arch_t).addModel()
                
                .partialState().with(ArchBlock.FACING, Direction.WEST).with(ArchBlock.TYPE, IntersectionShape.SINGLE)
                .modelForState().modelFile(arch).rotationY(270).uvLock(true).addModel()
                .partialState().with(ArchBlock.FACING, Direction.WEST).with(ArchBlock.TYPE, IntersectionShape.END)
                .modelForState().modelFile(arch_end).rotationY(270).uvLock(true).addModel()
                .partialState().with(ArchBlock.FACING, Direction.WEST).with(ArchBlock.TYPE, IntersectionShape.CONNECTED)
                .modelForState().modelFile(arch_connected).rotationY(270).uvLock(true).addModel()
                .partialState().with(ArchBlock.FACING, Direction.WEST).with(ArchBlock.TYPE, IntersectionShape.INNER_LEFT)
                .modelForState().modelFile(arch_corner).rotationY(270).uvLock(true).addModel()
                .partialState().with(ArchBlock.FACING, Direction.WEST).with(ArchBlock.TYPE, IntersectionShape.INNER_RIGHT)
                .modelForState().modelFile(arch_corner).rotationY(180).uvLock(true).addModel()
                .partialState().with(ArchBlock.FACING, Direction.WEST).with(ArchBlock.TYPE, IntersectionShape.OUTER_LEFT)
                .modelForState().modelFile(arch_corner).addModel()
                .partialState().with(ArchBlock.FACING, Direction.WEST).with(ArchBlock.TYPE, IntersectionShape.OUTER_RIGHT)
                .modelForState().modelFile(arch_corner).rotationY(90).uvLock(true).addModel()
                .partialState().with(ArchBlock.FACING, Direction.WEST).with(ArchBlock.TYPE, IntersectionShape.CROSS)
                .modelForState().modelFile(arch_cross).addModel()
                .partialState().with(ArchBlock.FACING, Direction.WEST).with(ArchBlock.TYPE, IntersectionShape.T)
                .modelForState().modelFile(arch_t).rotationY(270).uvLock(true).addModel()
                .partialState().with(ArchBlock.FACING, Direction.WEST).with(ArchBlock.TYPE, IntersectionShape.T_LEFT)
                .modelForState().modelFile(arch_t).addModel()
                .partialState().with(ArchBlock.FACING, Direction.WEST).with(ArchBlock.TYPE, IntersectionShape.T_RIGHT)
                .modelForState().modelFile(arch_t).rotationY(180).uvLock(true).addModel()
                .partialState().with(ArchBlock.FACING, Direction.WEST).with(ArchBlock.TYPE, IntersectionShape.T_OPPOSITE)
                .modelForState().modelFile(arch_t).rotationY(90).uvLock(true).addModel();
    }
    ...
    public void balustradeBlock(BalustradeBlock block, ResourceLocation side, ResourceLocation bottom, ResourceLocation top) {
        ModelFile balustrade = models().withExistingParent(name(block), modLoc("block/templates/balustrade"))
                ...
                .texture("top", top)
                .texture("bottom", bottom),
                balustrade_end = models().withExistingParent(name(block) + "_end", modLoc("block/templates/balustrade_end"))
                        .texture("side", side)
                        .texture("top", top)
                        .texture("bottom", bottom),
                balustrade_connected = models().withExistingParent(name(block) + "_connected", modLoc("block/templates/balustrade_connected"))
                        ...
                        .texture("top", top)
                        .texture("bottom", bottom),
                balustrade_corner = models().withExistingParent(name(block) + "_corner", modLoc("block/templates/balustrade_corner"))
                        .texture("side", side)
                        .texture("top", top)
                        .texture("bottom", bottom),
                balustrade_cross = models().withExistingParent(name(block) + "_cross", modLoc("block/templates/balustrade_cross"))
                        .texture("side", side)
                        .texture("top", top)
                        .texture("bottom", bottom),
                balustrade_t = models().withExistingParent(name(block) + "_t", modLoc("block/templates/balustrade_t"))
                        .texture("side", side)
                        .texture("top", top)
                        .texture("bottom", bottom);

        getVariantBuilder(block)
                .partialState().with(BalustradeBlock.FACING, Direction.NORTH).with(BalustradeBlock.TYPE, IntersectionShape.SINGLE)
                .modelForState().modelFile(balustrade).addModel()
                .partialState().with(BalustradeBlock.FACING, Direction.NORTH).with(BalustradeBlock.TYPE, IntersectionShape.END)
                .modelForState().modelFile(balustrade_end).addModel()
                .partialState().with(BalustradeBlock.FACING, Direction.NORTH).with(BalustradeBlock.TYPE, IntersectionShape.CONNECTED)
                .modelForState().modelFile(balustrade_connected).addModel()
                .partialState().with(BalustradeBlock.FACING, Direction.NORTH).with(BalustradeBlock.TYPE, IntersectionShape.INNER_LEFT)
                .modelForState().modelFile(balustrade_corner).addModel()
                .partialState().with(BalustradeBlock.FACING, Direction.NORTH).with(BalustradeBlock.TYPE, IntersectionShape.INNER_RIGHT)
                .modelForState().modelFile(balustrade_corner).rotationY(270).uvLock(true).addModel()
                .partialState().with(BalustradeBlock.FACING, Direction.NORTH).with(BalustradeBlock.TYPE, IntersectionShape.OUTER_LEFT)
                .modelForState().modelFile(balustrade_corner).rotationY(90).uvLock(true).addModel()
                .partialState().with(BalustradeBlock.FACING, Direction.NORTH).with(BalustradeBlock.TYPE, IntersectionShape.OUTER_RIGHT)
                .modelForState().modelFile(balustrade_corner).rotationY(180).uvLock(true).addModel()
                .partialState().with(BalustradeBlock.FACING, Direction.NORTH).with(BalustradeBlock.TYPE, IntersectionShape.CROSS)
                .modelForState().modelFile(balustrade_cross).addModel()
                .partialState().with(BalustradeBlock.FACING, Direction.NORTH).with(BalustradeBlock.TYPE, IntersectionShape.T)
                .modelForState().modelFile(balustrade_t).addModel()
                .partialState().with(BalustradeBlock.FACING, Direction.NORTH).with(BalustradeBlock.TYPE, IntersectionShape.T_LEFT)
                .modelForState().modelFile(balustrade_t).rotationY(90).uvLock(true).addModel()
                .partialState().with(BalustradeBlock.FACING, Direction.NORTH).with(BalustradeBlock.TYPE, IntersectionShape.T_RIGHT)
                .modelForState().modelFile(balustrade_t).rotationY(270).uvLock(true).addModel()
                .partialState().with(BalustradeBlock.FACING, Direction.NORTH).with(BalustradeBlock.TYPE, IntersectionShape.T_OPPOSITE)
                .modelForState().modelFile(balustrade_t).rotationY(180).uvLock(true).addModel()

                .partialState().with(BalustradeBlock.FACING, Direction.EAST).with(BalustradeBlock.TYPE, IntersectionShape.SINGLE)
                .modelForState().modelFile(balustrade).addModel()
                .partialState().with(BalustradeBlock.FACING, Direction.EAST).with(BalustradeBlock.TYPE, IntersectionShape.END)
                .modelForState().modelFile(balustrade_end).rotationY(90).uvLock(true).addModel()
                .partialState().with(BalustradeBlock.FACING, Direction.EAST).with(BalustradeBlock.TYPE, IntersectionShape.CONNECTED)
                .modelForState().modelFile(balustrade_connected).rotationY(90).uvLock(true).addModel()
                .partialState().with(BalustradeBlock.FACING, Direction.EAST).with(BalustradeBlock.TYPE, IntersectionShape.INNER_LEFT)
                .modelForState().modelFile(balustrade_corner).rotationY(90).uvLock(true).addModel()
                .partialState().with(BalustradeBlock.FACING, Direction.EAST).with(BalustradeBlock.TYPE, IntersectionShape.INNER_RIGHT)
                .modelForState().modelFile(balustrade_corner).addModel()
                .partialState().with(BalustradeBlock.FACING, Direction.EAST).with(BalustradeBlock.TYPE, IntersectionShape.OUTER_LEFT)
                .modelForState().modelFile(balustrade_corner).rotationY(180).uvLock(true).addModel()
                .partialState().with(BalustradeBlock.FACING, Direction.EAST).with(BalustradeBlock.TYPE, IntersectionShape.OUTER_RIGHT)
                .modelForState().modelFile(balustrade_corner).rotationY(270).uvLock(true).addModel()
                .partialState().with(BalustradeBlock.FACING, Direction.EAST).with(BalustradeBlock.TYPE, IntersectionShape.CROSS)
                .modelForState().modelFile(balustrade_cross).addModel()
                .partialState().with(BalustradeBlock.FACING, Direction.EAST).with(BalustradeBlock.TYPE, IntersectionShape.T)
                .modelForState().modelFile(balustrade_t).rotationY(90).uvLock(true).addModel()
                .partialState().with(BalustradeBlock.FACING, Direction.EAST).with(BalustradeBlock.TYPE, IntersectionShape.T_LEFT)
                .modelForState().modelFile(balustrade_t).rotationY(180).uvLock(true).addModel()
                .partialState().with(BalustradeBlock.FACING, Direction.EAST).with(BalustradeBlock.TYPE, IntersectionShape.T_RIGHT)
                .modelForState().modelFile(balustrade_t).addModel()
                .partialState().with(BalustradeBlock.FACING, Direction.EAST).with(BalustradeBlock.TYPE, IntersectionShape.T_OPPOSITE)
                .modelForState().modelFile(balustrade_t).rotationY(270).uvLock(true).addModel()
                
                .partialState().with(BalustradeBlock.FACING, Direction.SOUTH).with(BalustradeBlock.TYPE, IntersectionShape.SINGLE)
                .modelForState().modelFile(balustrade).addModel()
                .partialState().with(BalustradeBlock.FACING, Direction.SOUTH).with(BalustradeBlock.TYPE, IntersectionShape.END)
                .modelForState().modelFile(balustrade_end).rotationY(180).uvLock(true).addModel()
                .partialState().with(BalustradeBlock.FACING, Direction.SOUTH).with(BalustradeBlock.TYPE, IntersectionShape.CONNECTED)
                .modelForState().modelFile(balustrade_connected).rotationY(180).uvLock(true).addModel()
                .partialState().with(BalustradeBlock.FACING, Direction.SOUTH).with(BalustradeBlock.TYPE, IntersectionShape.INNER_LEFT)
                .modelForState().modelFile(balustrade_corner).rotationY(180).uvLock(true).addModel()
                .partialState().with(BalustradeBlock.FACING, Direction.SOUTH).with(BalustradeBlock.TYPE, IntersectionShape.INNER_RIGHT)
                .modelForState().modelFile(balustrade_corner).rotationY(90).uvLock(true).addModel()
                .partialState().with(BalustradeBlock.FACING, Direction.SOUTH).with(BalustradeBlock.TYPE, IntersectionShape.OUTER_LEFT)
                .modelForState().modelFile(balustrade_corner).rotationY(270).uvLock(true).addModel()
                .partialState().with(BalustradeBlock.FACING, Direction.SOUTH).with(BalustradeBlock.TYPE, IntersectionShape.OUTER_RIGHT)
                .modelForState().modelFile(balustrade_corner).addModel()
                .partialState().with(BalustradeBlock.FACING, Direction.SOUTH).with(BalustradeBlock.TYPE, IntersectionShape.CROSS)
                .modelForState().modelFile(balustrade_cross).addModel()
                .partialState().with(BalustradeBlock.FACING, Direction.SOUTH).with(BalustradeBlock.TYPE, IntersectionShape.T)
                .modelForState().modelFile(balustrade_t).rotationY(180).uvLock(true).addModel()
                .partialState().with(BalustradeBlock.FACING, Direction.SOUTH).with(BalustradeBlock.TYPE, IntersectionShape.T_LEFT)
                .modelForState().modelFile(balustrade_t).rotationY(270).uvLock(true).addModel()
                .partialState().with(BalustradeBlock.FACING, Direction.SOUTH).with(BalustradeBlock.TYPE, IntersectionShape.T_RIGHT)
                .modelForState().modelFile(balustrade_t).rotationY(90).uvLock(true).addModel()
                .partialState().with(BalustradeBlock.FACING, Direction.SOUTH).with(BalustradeBlock.TYPE, IntersectionShape.T_OPPOSITE)
                .modelForState().modelFile(balustrade_t).addModel()
                
                .partialState().with(BalustradeBlock.FACING, Direction.WEST).with(BalustradeBlock.TYPE, IntersectionShape.SINGLE)
                .modelForState().modelFile(balustrade).addModel()
                .partialState().with(BalustradeBlock.FACING, Direction.WEST).with(BalustradeBlock.TYPE, IntersectionShape.END)
                .modelForState().modelFile(balustrade_end).rotationY(270).uvLock(true).addModel()
                .partialState().with(BalustradeBlock.FACING, Direction.WEST).with(BalustradeBlock.TYPE, IntersectionShape.CONNECTED)
                .modelForState().modelFile(balustrade_connected).rotationY(270).uvLock(true).addModel()
                .partialState().with(BalustradeBlock.FACING, Direction.WEST).with(BalustradeBlock.TYPE, IntersectionShape.INNER_LEFT)
                .modelForState().modelFile(balustrade_corner).rotationY(270).uvLock(true).addModel()
                .partialState().with(BalustradeBlock.FACING, Direction.WEST).with(BalustradeBlock.TYPE, IntersectionShape.INNER_RIGHT)
                .modelForState().modelFile(balustrade_corner).rotationY(180).uvLock(true).addModel()
                .partialState().with(BalustradeBlock.FACING, Direction.WEST).with(BalustradeBlock.TYPE, IntersectionShape.OUTER_LEFT)
                .modelForState().modelFile(balustrade_corner).addModel()
                .partialState().with(BalustradeBlock.FACING, Direction.WEST).with(BalustradeBlock.TYPE, IntersectionShape.OUTER_RIGHT)
                .modelForState().modelFile(balustrade_corner).rotationY(90).uvLock(true).addModel()
                .partialState().with(BalustradeBlock.FACING, Direction.WEST).with(BalustradeBlock.TYPE, IntersectionShape.CROSS)
                .modelForState().modelFile(balustrade_cross).addModel()
                .partialState().with(BalustradeBlock.FACING, Direction.WEST).with(BalustradeBlock.TYPE, IntersectionShape.T)
                .modelForState().modelFile(balustrade_t).rotationY(270).uvLock(true).addModel()
                .partialState().with(BalustradeBlock.FACING, Direction.WEST).with(BalustradeBlock.TYPE, IntersectionShape.T_LEFT)
                .modelForState().modelFile(balustrade_t).addModel()
                .partialState().with(BalustradeBlock.FACING, Direction.WEST).with(BalustradeBlock.TYPE, IntersectionShape.T_RIGHT)
                .modelForState().modelFile(balustrade_t).rotationY(180).uvLock(true).addModel()
                .partialState().with(BalustradeBlock.FACING, Direction.WEST).with(BalustradeBlock.TYPE, IntersectionShape.T_OPPOSITE)
                .modelForState().modelFile(balustrade_t).rotationY(90).uvLock(true).addModel();
    }
    ...
    public void beamLintelBlock(BeamLintelBlock block, ResourceLocation side, ResourceLocation bottom, ResourceLocation top) {
        ...
        getVariantBuilder(block)
                ...
                .partialState().with(BeamLintelBlock.FACING, Direction.NORTH).with(BeamLintelBlock.HALF, TopBottomShape.TOP).with(BeamLintelBlock.BEAM, 1)
                .modelForState().modelFile(beam_lintel_1).addModel()
                .partialState().with(BeamLintelBlock.FACING, Direction.NORTH).with(BeamLintelBlock.HALF, TopBottomShape.TOP).with(BeamLintelBlock.BEAM, 2)
                .modelForState().modelFile(beam_lintel_2).addModel()
                .partialState().with(BeamLintelBlock.FACING, Direction.NORTH).with(BeamLintelBlock.HALF, TopBottomShape.TOP).with(BeamLintelBlock.BEAM, 3)
                .modelForState().modelFile(beam_lintel_3).addModel()
                .partialState().with(BeamLintelBlock.FACING, Direction.NORTH).with(BeamLintelBlock.HALF, TopBottomShape.TOP).with(BeamLintelBlock.BEAM, 4)
                .modelForState().modelFile(beam_lintel_4).addModel()
                .partialState().with(BeamLintelBlock.FACING, Direction.NORTH).with(BeamLintelBlock.HALF, TopBottomShape.BOTTOM).with(BeamLintelBlock.BEAM, 1)
                .modelForState().modelFile(beam_lintel_1).rotationX(180).rotationY(180).addModel()
                .partialState().with(BeamLintelBlock.FACING, Direction.NORTH).with(BeamLintelBlock.HALF, TopBottomShape.BOTTOM).with(BeamLintelBlock.BEAM, 2)
                .modelForState().modelFile(beam_lintel_2).rotationX(180).rotationY(180).addModel()
                .partialState().with(BeamLintelBlock.FACING, Direction.NORTH).with(BeamLintelBlock.HALF, TopBottomShape.BOTTOM).with(BeamLintelBlock.BEAM, 3)
                .modelForState().modelFile(beam_lintel_3).rotationX(180).rotationY(180).addModel()
                .partialState().with(BeamLintelBlock.FACING, Direction.NORTH).with(BeamLintelBlock.HALF, TopBottomShape.BOTTOM).with(BeamLintelBlock.BEAM, 4)
                .modelForState().modelFile(beam_lintel_4).rotationX(180).rotationY(180).addModel()
                .partialState().with(BeamLintelBlock.FACING, Direction.EAST).with(BeamLintelBlock.HALF, TopBottomShape.TOP).with(BeamLintelBlock.BEAM, 1)
                .modelForState().modelFile(beam_lintel_1).rotationY(90).addModel()
                .partialState().with(BeamLintelBlock.FACING, Direction.EAST).with(BeamLintelBlock.HALF, TopBottomShape.TOP).with(BeamLintelBlock.BEAM, 2)
                .modelForState().modelFile(beam_lintel_2).rotationY(90).addModel()
                .partialState().with(BeamLintelBlock.FACING, Direction.EAST).with(BeamLintelBlock.HALF, TopBottomShape.TOP).with(BeamLintelBlock.BEAM, 3)
                .modelForState().modelFile(beam_lintel_3).rotationY(90).addModel()
                .partialState().with(BeamLintelBlock.FACING, Direction.EAST).with(BeamLintelBlock.HALF, TopBottomShape.TOP).with(BeamLintelBlock.BEAM, 4)
                .modelForState().modelFile(beam_lintel_4).rotationY(90).addModel()
                .partialState().with(BeamLintelBlock.FACING, Direction.EAST).with(BeamLintelBlock.HALF, TopBottomShape.BOTTOM).with(BeamLintelBlock.BEAM, 1)
                .modelForState().modelFile(beam_lintel_1).rotationX(180).rotationY(270).addModel()
                .partialState().with(BeamLintelBlock.FACING, Direction.EAST).with(BeamLintelBlock.HALF, TopBottomShape.BOTTOM).with(BeamLintelBlock.BEAM, 2)
                .modelForState().modelFile(beam_lintel_2).rotationX(180).rotationY(270).addModel()
                .partialState().with(BeamLintelBlock.FACING, Direction.EAST).with(BeamLintelBlock.HALF, TopBottomShape.BOTTOM).with(BeamLintelBlock.BEAM, 3)
                .modelForState().modelFile(beam_lintel_3).rotationX(180).rotationY(270).addModel()
                .partialState().with(BeamLintelBlock.FACING, Direction.EAST).with(BeamLintelBlock.HALF, TopBottomShape.BOTTOM).with(BeamLintelBlock.BEAM, 4)
                .modelForState().modelFile(beam_lintel_4).rotationX(180).rotationY(270).addModel()
                .partialState().with(BeamLintelBlock.FACING, Direction.SOUTH).with(BeamLintelBlock.HALF, TopBottomShape.TOP).with(BeamLintelBlock.BEAM, 1)
                .modelForState().modelFile(beam_lintel_1).rotationY(180).addModel()
                .partialState().with(BeamLintelBlock.FACING, Direction.SOUTH).with(BeamLintelBlock.HALF, TopBottomShape.TOP).with(BeamLintelBlock.BEAM, 2)
                .modelForState().modelFile(beam_lintel_2).rotationY(180).addModel()
                .partialState().with(BeamLintelBlock.FACING, Direction.SOUTH).with(BeamLintelBlock.HALF, TopBottomShape.TOP).with(BeamLintelBlock.BEAM, 3)
                .modelForState().modelFile(beam_lintel_3).rotationY(180).addModel()
                .partialState().with(BeamLintelBlock.FACING, Direction.SOUTH).with(BeamLintelBlock.HALF, TopBottomShape.TOP).with(BeamLintelBlock.BEAM, 4)
                .modelForState().modelFile(beam_lintel_4).rotationY(180).addModel()
                .partialState().with(BeamLintelBlock.FACING, Direction.SOUTH).with(BeamLintelBlock.HALF, TopBottomShape.BOTTOM).with(BeamLintelBlock.BEAM, 1)
                .modelForState().modelFile(beam_lintel_1).rotationX(180).addModel()
                .partialState().with(BeamLintelBlock.FACING, Direction.SOUTH).with(BeamLintelBlock.HALF, TopBottomShape.BOTTOM).with(BeamLintelBlock.BEAM, 2)
                .modelForState().modelFile(beam_lintel_2).rotationX(180).addModel()
                .partialState().with(BeamLintelBlock.FACING, Direction.SOUTH).with(BeamLintelBlock.HALF, TopBottomShape.BOTTOM).with(BeamLintelBlock.BEAM, 3)
                .modelForState().modelFile(beam_lintel_3).rotationX(180).addModel()
                .partialState().with(BeamLintelBlock.FACING, Direction.SOUTH).with(BeamLintelBlock.HALF, TopBottomShape.BOTTOM).with(BeamLintelBlock.BEAM, 4)
                .modelForState().modelFile(beam_lintel_4).rotationX(180).addModel()
                .partialState().with(BeamLintelBlock.FACING, Direction.WEST).with(BeamLintelBlock.HALF, TopBottomShape.TOP).with(BeamLintelBlock.BEAM, 1)
                .modelForState().modelFile(beam_lintel_1).rotationY(270).addModel()
                .partialState().with(BeamLintelBlock.FACING, Direction.WEST).with(BeamLintelBlock.HALF, TopBottomShape.TOP).with(BeamLintelBlock.BEAM, 2)
                .modelForState().modelFile(beam_lintel_2).rotationY(270).addModel()
                .partialState().with(BeamLintelBlock.FACING, Direction.WEST).with(BeamLintelBlock.HALF, TopBottomShape.TOP).with(BeamLintelBlock.BEAM, 3)
                .modelForState().modelFile(beam_lintel_3).rotationY(270).addModel()
                .partialState().with(BeamLintelBlock.FACING, Direction.WEST).with(BeamLintelBlock.HALF, TopBottomShape.TOP).with(BeamLintelBlock.BEAM, 4)
                .modelForState().modelFile(beam_lintel_4).rotationY(270).addModel()
                .partialState().with(BeamLintelBlock.FACING, Direction.WEST).with(BeamLintelBlock.HALF, TopBottomShape.BOTTOM).with(BeamLintelBlock.BEAM, 1)
                .modelForState().modelFile(beam_lintel_1).rotationX(180).rotationY(90).addModel()
                .partialState().with(BeamLintelBlock.FACING, Direction.WEST).with(BeamLintelBlock.HALF, TopBottomShape.BOTTOM).with(BeamLintelBlock.BEAM, 2)
                .modelForState().modelFile(beam_lintel_2).rotationX(180).rotationY(90).addModel()
                .partialState().with(BeamLintelBlock.FACING, Direction.WEST).with(BeamLintelBlock.HALF, TopBottomShape.BOTTOM).with(BeamLintelBlock.BEAM, 3)
                .modelForState().modelFile(beam_lintel_3).rotationX(180).rotationY(90).addModel()
                .partialState().with(BeamLintelBlock.FACING, Direction.WEST).with(BeamLintelBlock.HALF, TopBottomShape.BOTTOM).with(BeamLintelBlock.BEAM, 4)
    ...
    public void beamHorizontalBlock(HorizontalBeamBlock block, ResourceLocation side, ResourceLocation bottom, ResourceLocation top) {
        ...
        getVariantBuilder(block)
                .partialState().with(HorizontalBeamBlock.FACING, Direction.NORTH).with(HorizontalBeamBlock.HALF, TopBottomShape.TOP).with(HorizontalBeamBlock.BEAM, 1)
                .modelForState().modelFile(beam_horizontal_1).rotationX(180).addModel()
                .partialState().with(HorizontalBeamBlock.FACING, Direction.NORTH).with(HorizontalBeamBlock.HALF, TopBottomShape.TOP).with(HorizontalBeamBlock.BEAM, 2)
                .modelForState().modelFile(beam_horizontal_2).rotationX(180).rotationY(90).addModel()
                .partialState().with(HorizontalBeamBlock.FACING, Direction.NORTH).with(HorizontalBeamBlock.HALF, TopBottomShape.TOP).with(HorizontalBeamBlock.BEAM, 3)
                .modelForState().modelFile(beam_horizontal_3).rotationX(180).rotationY(270).addModel()
                .partialState().with(HorizontalBeamBlock.FACING, Direction.NORTH).with(HorizontalBeamBlock.HALF, TopBottomShape.TOP).with(HorizontalBeamBlock.BEAM, 4)
                .modelForState().modelFile(beam_horizontal_4).rotationX(180).addModel()
                .partialState().with(HorizontalBeamBlock.FACING, Direction.NORTH).with(HorizontalBeamBlock.HALF, TopBottomShape.TOP).with(HorizontalBeamBlock.BEAM, 5)
                .modelForState().modelFile(beam_horizontal_5).rotationX(180).addModel()
                .partialState().with(HorizontalBeamBlock.FACING, Direction.NORTH).with(HorizontalBeamBlock.HALF, TopBottomShape.TOP).with(HorizontalBeamBlock.BEAM, 6)
                .modelForState().modelFile(beam_horizontal_6).rotationX(180).addModel()
                .partialState().with(HorizontalBeamBlock.FACING, Direction.NORTH).with(HorizontalBeamBlock.HALF, TopBottomShape.BOTTOM).with(HorizontalBeamBlock.BEAM, 1)
                .modelForState().modelFile(beam_horizontal_1).addModel()
                .partialState().with(HorizontalBeamBlock.FACING, Direction.NORTH).with(HorizontalBeamBlock.HALF, TopBottomShape.BOTTOM).with(HorizontalBeamBlock.BEAM, 2)
                .modelForState().modelFile(beam_horizontal_2).addModel()
                .partialState().with(HorizontalBeamBlock.FACING, Direction.NORTH).with(HorizontalBeamBlock.HALF, TopBottomShape.BOTTOM).with(HorizontalBeamBlock.BEAM, 3)
                .modelForState().modelFile(beam_horizontal_3).addModel()
                .partialState().with(HorizontalBeamBlock.FACING, Direction.NORTH).with(HorizontalBeamBlock.HALF, TopBottomShape.BOTTOM).with(HorizontalBeamBlock.BEAM, 4)
                .modelForState().modelFile(beam_horizontal_4).addModel()
                .partialState().with(HorizontalBeamBlock.FACING, Direction.NORTH).with(HorizontalBeamBlock.HALF, TopBottomShape.BOTTOM).with(HorizontalBeamBlock.BEAM, 5)
                .modelForState().modelFile(beam_horizontal_5).addModel()
                .partialState().with(HorizontalBeamBlock.FACING, Direction.NORTH).with(HorizontalBeamBlock.HALF, TopBottomShape.BOTTOM).with(HorizontalBeamBlock.BEAM, 6)
                .modelForState().modelFile(beam_horizontal_6).addModel()
                .partialState().with(HorizontalBeamBlock.FACING, Direction.EAST).with(HorizontalBeamBlock.HALF, TopBottomShape.TOP).with(HorizontalBeamBlock.BEAM, 1)
                .modelForState().modelFile(beam_horizontal_1).rotationX(180).rotationY(90).addModel()
                .partialState().with(HorizontalBeamBlock.FACING, Direction.EAST).with(HorizontalBeamBlock.HALF, TopBottomShape.TOP).with(HorizontalBeamBlock.BEAM, 2)
                .modelForState().modelFile(beam_horizontal_2).rotationX(180).rotationY(180).addModel()
                .partialState().with(HorizontalBeamBlock.FACING, Direction.EAST).with(HorizontalBeamBlock.HALF, TopBottomShape.TOP).with(HorizontalBeamBlock.BEAM, 3)
                .modelForState().modelFile(beam_horizontal_3).rotationX(180).addModel()
                .partialState().with(HorizontalBeamBlock.FACING, Direction.EAST).with(HorizontalBeamBlock.HALF, TopBottomShape.TOP).with(HorizontalBeamBlock.BEAM, 4)
                .modelForState().modelFile(beam_horizontal_4).rotationX(180).rotationY(90).addModel()
                .partialState().with(HorizontalBeamBlock.FACING, Direction.EAST).with(HorizontalBeamBlock.HALF, TopBottomShape.TOP).with(HorizontalBeamBlock.BEAM, 5)
                .modelForState().modelFile(beam_horizontal_5).rotationX(180).rotationY(90).addModel()
                .partialState().with(HorizontalBeamBlock.FACING, Direction.EAST).with(HorizontalBeamBlock.HALF, TopBottomShape.TOP).with(HorizontalBeamBlock.BEAM, 6)
                .modelForState().modelFile(beam_horizontal_6).rotationX(180).rotationY(90).addModel()
                .partialState().with(HorizontalBeamBlock.FACING, Direction.EAST).with(HorizontalBeamBlock.HALF, TopBottomShape.BOTTOM).with(HorizontalBeamBlock.BEAM, 1)
                .modelForState().modelFile(beam_horizontal_1).rotationY(90).addModel()
                .partialState().with(HorizontalBeamBlock.FACING, Direction.EAST).with(HorizontalBeamBlock.HALF, TopBottomShape.BOTTOM).with(HorizontalBeamBlock.BEAM, 2)
                .modelForState().modelFile(beam_horizontal_2).rotationY(90).addModel()
                .partialState().with(HorizontalBeamBlock.FACING, Direction.EAST).with(HorizontalBeamBlock.HALF, TopBottomShape.BOTTOM).with(HorizontalBeamBlock.BEAM, 3)
                .modelForState().modelFile(beam_horizontal_3).rotationY(90).addModel()
                .partialState().with(HorizontalBeamBlock.FACING, Direction.EAST).with(HorizontalBeamBlock.HALF, TopBottomShape.BOTTOM).with(HorizontalBeamBlock.BEAM, 4)
                .modelForState().modelFile(beam_horizontal_4).rotationY(90).addModel()
                .partialState().with(HorizontalBeamBlock.FACING, Direction.EAST).with(HorizontalBeamBlock.HALF, TopBottomShape.BOTTOM).with(HorizontalBeamBlock.BEAM, 5)
                .modelForState().modelFile(beam_horizontal_5).rotationY(90).addModel()
                .partialState().with(HorizontalBeamBlock.FACING, Direction.EAST).with(HorizontalBeamBlock.HALF, TopBottomShape.BOTTOM).with(HorizontalBeamBlock.BEAM, 6)
                .modelForState().modelFile(beam_horizontal_6).rotationY(90).addModel()
                .partialState().with(HorizontalBeamBlock.FACING, Direction.SOUTH).with(HorizontalBeamBlock.HALF, TopBottomShape.TOP).with(HorizontalBeamBlock.BEAM, 1)
                .modelForState().modelFile(beam_horizontal_1).rotationX(180).rotationY(180).addModel()
                .partialState().with(HorizontalBeamBlock.FACING, Direction.SOUTH).with(HorizontalBeamBlock.HALF, TopBottomShape.TOP).with(HorizontalBeamBlock.BEAM, 2)
                .modelForState().modelFile(beam_horizontal_2).rotationX(180).rotationY(270).addModel()
                .partialState().with(HorizontalBeamBlock.FACING, Direction.SOUTH).with(HorizontalBeamBlock.HALF, TopBottomShape.TOP).with(HorizontalBeamBlock.BEAM, 3)
                .modelForState().modelFile(beam_horizontal_3).rotationX(180).rotationY(90).addModel()
                .partialState().with(HorizontalBeamBlock.FACING, Direction.SOUTH).with(HorizontalBeamBlock.HALF, TopBottomShape.TOP).with(HorizontalBeamBlock.BEAM, 4)
                .modelForState().modelFile(beam_horizontal_4).rotationX(180).rotationY(180).addModel()
                .partialState().with(HorizontalBeamBlock.FACING, Direction.SOUTH).with(HorizontalBeamBlock.HALF, TopBottomShape.TOP).with(HorizontalBeamBlock.BEAM, 5)
                .modelForState().modelFile(beam_horizontal_5).rotationX(180).rotationY(180).addModel()
                .partialState().with(HorizontalBeamBlock.FACING, Direction.SOUTH).with(HorizontalBeamBlock.HALF, TopBottomShape.TOP).with(HorizontalBeamBlock.BEAM, 6)
                .modelForState().modelFile(beam_horizontal_6).rotationX(180).rotationY(180).addModel()
                .partialState().with(HorizontalBeamBlock.FACING, Direction.SOUTH).with(HorizontalBeamBlock.HALF, TopBottomShape.BOTTOM).with(HorizontalBeamBlock.BEAM, 1)
                .modelForState().modelFile(beam_horizontal_1).rotationY(180).addModel()
                .partialState().with(HorizontalBeamBlock.FACING, Direction.SOUTH).with(HorizontalBeamBlock.HALF, TopBottomShape.BOTTOM).with(HorizontalBeamBlock.BEAM, 2)
                .modelForState().modelFile(beam_horizontal_2).rotationY(180).addModel()
                .partialState().with(HorizontalBeamBlock.FACING, Direction.SOUTH).with(HorizontalBeamBlock.HALF, TopBottomShape.BOTTOM).with(HorizontalBeamBlock.BEAM, 3)
                .modelForState().modelFile(beam_horizontal_3).rotationY(180).addModel()
                .partialState().with(HorizontalBeamBlock.FACING, Direction.SOUTH).with(HorizontalBeamBlock.HALF, TopBottomShape.BOTTOM).with(HorizontalBeamBlock.BEAM, 4)
                .modelForState().modelFile(beam_horizontal_4).rotationY(180).addModel()
                .partialState().with(HorizontalBeamBlock.FACING, Direction.SOUTH).with(HorizontalBeamBlock.HALF, TopBottomShape.BOTTOM).with(HorizontalBeamBlock.BEAM, 5)
                .modelForState().modelFile(beam_horizontal_5).rotationY(180).addModel()
                .partialState().with(HorizontalBeamBlock.FACING, Direction.SOUTH).with(HorizontalBeamBlock.HALF, TopBottomShape.BOTTOM).with(HorizontalBeamBlock.BEAM, 6)
                .modelForState().modelFile(beam_horizontal_6).rotationY(180).addModel()
                .partialState().with(HorizontalBeamBlock.FACING, Direction.WEST).with(HorizontalBeamBlock.HALF, TopBottomShape.TOP).with(HorizontalBeamBlock.BEAM, 1)
                .modelForState().modelFile(beam_horizontal_1).rotationX(180).rotationY(270).addModel()
                .partialState().with(HorizontalBeamBlock.FACING, Direction.WEST).with(HorizontalBeamBlock.HALF, TopBottomShape.TOP).with(HorizontalBeamBlock.BEAM, 2)
                .modelForState().modelFile(beam_horizontal_2).rotationX(180).addModel()
                .partialState().with(HorizontalBeamBlock.FACING, Direction.WEST).with(HorizontalBeamBlock.HALF, TopBottomShape.TOP).with(HorizontalBeamBlock.BEAM, 3)
                .modelForState().modelFile(beam_horizontal_3).rotationX(180).rotationY(180).addModel()
                .partialState().with(HorizontalBeamBlock.FACING, Direction.WEST).with(HorizontalBeamBlock.HALF, TopBottomShape.TOP).with(HorizontalBeamBlock.BEAM, 4)
                .modelForState().modelFile(beam_horizontal_4).rotationX(180).rotationY(270).addModel()
                .partialState().with(HorizontalBeamBlock.FACING, Direction.WEST).with(HorizontalBeamBlock.HALF, TopBottomShape.TOP).with(HorizontalBeamBlock.BEAM, 5)
                .modelForState().modelFile(beam_horizontal_5).rotationX(180).rotationY(270).addModel()
                .partialState().with(HorizontalBeamBlock.FACING, Direction.WEST).with(HorizontalBeamBlock.HALF, TopBottomShape.TOP).with(HorizontalBeamBlock.BEAM, 6)
                .modelForState().modelFile(beam_horizontal_6).rotationX(180).rotationY(270).addModel()
                .partialState().with(HorizontalBeamBlock.FACING, Direction.WEST).with(HorizontalBeamBlock.HALF, TopBottomShape.BOTTOM).with(HorizontalBeamBlock.BEAM, 1)
                .modelForState().modelFile(beam_horizontal_1).rotationY(270).addModel()
                .partialState().with(HorizontalBeamBlock.FACING, Direction.WEST).with(HorizontalBeamBlock.HALF, TopBottomShape.BOTTOM).with(HorizontalBeamBlock.BEAM, 2)
                .modelForState().modelFile(beam_horizontal_2).rotationY(270).addModel()
                .partialState().with(HorizontalBeamBlock.FACING, Direction.WEST).with(HorizontalBeamBlock.HALF, TopBottomShape.BOTTOM).with(HorizontalBeamBlock.BEAM, 3)
                .modelForState().modelFile(beam_horizontal_3).rotationY(270).addModel()
                .partialState().with(HorizontalBeamBlock.FACING, Direction.WEST).with(HorizontalBeamBlock.HALF, TopBottomShape.BOTTOM).with(HorizontalBeamBlock.BEAM, 4)
                .modelForState().modelFile(beam_horizontal_4).rotationY(270).addModel()
                .partialState().with(HorizontalBeamBlock.FACING, Direction.WEST).with(HorizontalBeamBlock.HALF, TopBottomShape.BOTTOM).with(HorizontalBeamBlock.BEAM, 5)
                .modelForState().modelFile(beam_horizontal_5).rotationY(270).addModel()
                .partialState().with(HorizontalBeamBlock.FACING, Direction.WEST).with(HorizontalBeamBlock.HALF, TopBottomShape.BOTTOM).with(HorizontalBeamBlock.BEAM, 6)
    ...
    public void capitalBlock(CapitalBlock block, ResourceLocation side, ResourceLocation bottom, ResourceLocation top) {
        ModelFile capital = models().withExistingParent(name(block), modLoc("block/templates/capital"))
                ...
                .texture("top", top)
                .texture("bottom", bottom),
                capital_end = models().withExistingParent(name(block) + "_end", modLoc("block/templates/capital_end"))
                        .texture("side", side)
                        .texture("top", top)
                        .texture("bottom", bottom),
                capital_connected = models().withExistingParent(name(block) + "_connected", modLoc("block/templates/capital_connected"))
                        .texture("side", side)
                        .texture("top", top)
                        .texture("bottom", bottom),
                capital_corner = models().withExistingParent(name(block) + "_corner", modLoc("block/templates/capital_corner"))
                        .texture("side", side)
                        .texture("top", top)
                        .texture("bottom", bottom),
                capital_cross = models().withExistingParent(name(block) + "_cross", modLoc("block/templates/capital_cross"))
                        .texture("side", side)
                        .texture("top", top)
                        .texture("bottom", bottom),
                capital_t = models().withExistingParent(name(block) + "_t", modLoc("block/templates/capital_t"))
                        .texture("side", side)
                        .texture("top", top)
                        .texture("bottom", bottom),
                capital_top = models().withExistingParent(name(block) + "_top", modLoc("block/templates/capital_top"))
                        ...
                        .texture("top", top)
                        .texture("bottom", bottom),
                capital_top_end = models().withExistingParent(name(block) + "_top_end", modLoc("block/templates/capital_top_end"))
                        .texture("side", side)
                        .texture("top", top)
                        .texture("bottom", bottom),
                capital_top_connected = models().withExistingParent(name(block) + "_top_connected", modLoc("block/templates/capital_top_connected"))
                        .texture("side", side)
                        .texture("top", top)
                        .texture("bottom", bottom),
                capital_top_corner = models().withExistingParent(name(block) + "_top_corner", modLoc("block/templates/capital_top_corner"))
                        .texture("side", side)
                        .texture("top", top)
                        .texture("bottom", bottom),
                capital_top_cross = models().withExistingParent(name(block) + "_top_cross", modLoc("block/templates/capital_top_cross"))
                        .texture("side", side)
                        .texture("top", top)
                        .texture("bottom", bottom),
                capital_top_t = models().withExistingParent(name(block) + "_top_t", modLoc("block/templates/capital_top_t"))
                        .texture("side", side)
                        .texture("top", top)
                        .texture("bottom", bottom);
        ...
        getVariantBuilder(block)
                // Bottom
                .partialState().with(CapitalBlock.FACING, Direction.NORTH).with(CapitalBlock.HALF, TopBottomShape.BOTTOM).with(CapitalBlock.TYPE, IntersectionShape.SINGLE)
                .modelForState().modelFile(capital).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.NORTH).with(CapitalBlock.HALF, TopBottomShape.BOTTOM).with(CapitalBlock.TYPE, IntersectionShape.END)
                .modelForState().modelFile(capital_end).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.NORTH).with(CapitalBlock.HALF, TopBottomShape.BOTTOM).with(CapitalBlock.TYPE, IntersectionShape.CONNECTED)
                .modelForState().modelFile(capital_connected).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.NORTH).with(CapitalBlock.HALF, TopBottomShape.BOTTOM).with(CapitalBlock.TYPE, IntersectionShape.INNER_LEFT)
                .modelForState().modelFile(capital_corner).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.NORTH).with(CapitalBlock.HALF, TopBottomShape.BOTTOM).with(CapitalBlock.TYPE, IntersectionShape.INNER_RIGHT)
                .modelForState().modelFile(capital_corner).rotationY(270).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.NORTH).with(CapitalBlock.HALF, TopBottomShape.BOTTOM).with(CapitalBlock.TYPE, IntersectionShape.OUTER_LEFT)
                .modelForState().modelFile(capital_corner).rotationY(90).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.NORTH).with(CapitalBlock.HALF, TopBottomShape.BOTTOM).with(CapitalBlock.TYPE, IntersectionShape.OUTER_RIGHT)
                .modelForState().modelFile(capital_corner).rotationY(180).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.NORTH).with(CapitalBlock.HALF, TopBottomShape.BOTTOM).with(CapitalBlock.TYPE, IntersectionShape.CROSS)
                .modelForState().modelFile(capital_cross).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.NORTH).with(CapitalBlock.HALF, TopBottomShape.BOTTOM).with(CapitalBlock.TYPE, IntersectionShape.T)
                .modelForState().modelFile(capital_t).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.NORTH).with(CapitalBlock.HALF, TopBottomShape.BOTTOM).with(CapitalBlock.TYPE, IntersectionShape.T_LEFT)
                .modelForState().modelFile(capital_t).rotationY(90).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.NORTH).with(CapitalBlock.HALF, TopBottomShape.BOTTOM).with(CapitalBlock.TYPE, IntersectionShape.T_RIGHT)
                .modelForState().modelFile(capital_t).rotationY(270).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.NORTH).with(CapitalBlock.HALF, TopBottomShape.BOTTOM).with(CapitalBlock.TYPE, IntersectionShape.T_OPPOSITE)
                .modelForState().modelFile(capital_t).rotationY(180).uvLock(true).addModel()
                // Top
                .partialState().with(CapitalBlock.FACING, Direction.NORTH).with(CapitalBlock.HALF, TopBottomShape.TOP).with(CapitalBlock.TYPE, IntersectionShape.SINGLE)
                .modelForState().modelFile(capital_top).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.NORTH).with(CapitalBlock.HALF, TopBottomShape.TOP).with(CapitalBlock.TYPE, IntersectionShape.END)
                .modelForState().modelFile(capital_top_end).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.NORTH).with(CapitalBlock.HALF, TopBottomShape.TOP).with(CapitalBlock.TYPE, IntersectionShape.CONNECTED)
                .modelForState().modelFile(capital_top_connected).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.NORTH).with(CapitalBlock.HALF, TopBottomShape.TOP).with(CapitalBlock.TYPE, IntersectionShape.INNER_LEFT)
                .modelForState().modelFile(capital_top_corner).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.NORTH).with(CapitalBlock.HALF, TopBottomShape.TOP).with(CapitalBlock.TYPE, IntersectionShape.INNER_RIGHT)
                .modelForState().modelFile(capital_top_corner).rotationY(270).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.NORTH).with(CapitalBlock.HALF, TopBottomShape.TOP).with(CapitalBlock.TYPE, IntersectionShape.OUTER_LEFT)
                .modelForState().modelFile(capital_top_corner).rotationY(90).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.NORTH).with(CapitalBlock.HALF, TopBottomShape.TOP).with(CapitalBlock.TYPE, IntersectionShape.OUTER_RIGHT)
                .modelForState().modelFile(capital_top_corner).rotationY(180).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.NORTH).with(CapitalBlock.HALF, TopBottomShape.TOP).with(CapitalBlock.TYPE, IntersectionShape.CROSS)
                .modelForState().modelFile(capital_top_cross).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.NORTH).with(CapitalBlock.HALF, TopBottomShape.TOP).with(CapitalBlock.TYPE, IntersectionShape.T)
                .modelForState().modelFile(capital_top_t).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.NORTH).with(CapitalBlock.HALF, TopBottomShape.TOP).with(CapitalBlock.TYPE, IntersectionShape.T_LEFT)
                .modelForState().modelFile(capital_top_t).rotationY(90).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.NORTH).with(CapitalBlock.HALF, TopBottomShape.TOP).with(CapitalBlock.TYPE, IntersectionShape.T_RIGHT)
                .modelForState().modelFile(capital_top_t).rotationY(270).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.NORTH).with(CapitalBlock.HALF, TopBottomShape.TOP).with(CapitalBlock.TYPE, IntersectionShape.T_OPPOSITE)
                .modelForState().modelFile(capital_top_t).rotationY(180).uvLock(true).addModel()

                // Bottom
                .partialState().with(CapitalBlock.FACING, Direction.EAST).with(CapitalBlock.HALF, TopBottomShape.BOTTOM).with(CapitalBlock.TYPE, IntersectionShape.SINGLE)
                .modelForState().modelFile(capital).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.EAST).with(CapitalBlock.HALF, TopBottomShape.BOTTOM).with(CapitalBlock.TYPE, IntersectionShape.END)
                .modelForState().modelFile(capital_end).rotationY(90).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.EAST).with(CapitalBlock.HALF, TopBottomShape.BOTTOM).with(CapitalBlock.TYPE, IntersectionShape.CONNECTED)
                .modelForState().modelFile(capital_connected).rotationY(90).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.EAST).with(CapitalBlock.HALF, TopBottomShape.BOTTOM).with(CapitalBlock.TYPE, IntersectionShape.INNER_LEFT)
                .modelForState().modelFile(capital_corner).rotationY(90).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.EAST).with(CapitalBlock.HALF, TopBottomShape.BOTTOM).with(CapitalBlock.TYPE, IntersectionShape.INNER_RIGHT)
                .modelForState().modelFile(capital_corner).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.EAST).with(CapitalBlock.HALF, TopBottomShape.BOTTOM).with(CapitalBlock.TYPE, IntersectionShape.OUTER_LEFT)
                .modelForState().modelFile(capital_corner).rotationY(180).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.EAST).with(CapitalBlock.HALF, TopBottomShape.BOTTOM).with(CapitalBlock.TYPE, IntersectionShape.OUTER_RIGHT)
                .modelForState().modelFile(capital_corner).rotationY(270).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.EAST).with(CapitalBlock.HALF, TopBottomShape.BOTTOM).with(CapitalBlock.TYPE, IntersectionShape.CROSS)
                .modelForState().modelFile(capital_cross).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.EAST).with(CapitalBlock.HALF, TopBottomShape.BOTTOM).with(CapitalBlock.TYPE, IntersectionShape.T)
                .modelForState().modelFile(capital_t).rotationY(90).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.EAST).with(CapitalBlock.HALF, TopBottomShape.BOTTOM).with(CapitalBlock.TYPE, IntersectionShape.T_LEFT)
                .modelForState().modelFile(capital_t).rotationY(180).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.EAST).with(CapitalBlock.HALF, TopBottomShape.BOTTOM).with(CapitalBlock.TYPE, IntersectionShape.T_RIGHT)
                .modelForState().modelFile(capital_t).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.EAST).with(CapitalBlock.HALF, TopBottomShape.BOTTOM).with(CapitalBlock.TYPE, IntersectionShape.T_OPPOSITE)
                .modelForState().modelFile(capital_t).rotationY(270).uvLock(true).addModel()
                // Top
                .partialState().with(CapitalBlock.FACING, Direction.EAST).with(CapitalBlock.HALF, TopBottomShape.TOP).with(CapitalBlock.TYPE, IntersectionShape.SINGLE)
                .modelForState().modelFile(capital_top).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.EAST).with(CapitalBlock.HALF, TopBottomShape.TOP).with(CapitalBlock.TYPE, IntersectionShape.END)
                .modelForState().modelFile(capital_top_end).rotationY(90).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.EAST).with(CapitalBlock.HALF, TopBottomShape.TOP).with(CapitalBlock.TYPE, IntersectionShape.CONNECTED)
                .modelForState().modelFile(capital_top_connected).rotationY(90).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.EAST).with(CapitalBlock.HALF, TopBottomShape.TOP).with(CapitalBlock.TYPE, IntersectionShape.INNER_LEFT)
                .modelForState().modelFile(capital_top_corner).rotationY(90).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.EAST).with(CapitalBlock.HALF, TopBottomShape.TOP).with(CapitalBlock.TYPE, IntersectionShape.INNER_RIGHT)
                .modelForState().modelFile(capital_top_corner).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.EAST).with(CapitalBlock.HALF, TopBottomShape.TOP).with(CapitalBlock.TYPE, IntersectionShape.OUTER_LEFT)
                .modelForState().modelFile(capital_top_corner).rotationY(180).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.EAST).with(CapitalBlock.HALF, TopBottomShape.TOP).with(CapitalBlock.TYPE, IntersectionShape.OUTER_RIGHT)
                .modelForState().modelFile(capital_top_corner).rotationY(270).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.EAST).with(CapitalBlock.HALF, TopBottomShape.TOP).with(CapitalBlock.TYPE, IntersectionShape.CROSS)
                .modelForState().modelFile(capital_top_cross).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.EAST).with(CapitalBlock.HALF, TopBottomShape.TOP).with(CapitalBlock.TYPE, IntersectionShape.T)
                .modelForState().modelFile(capital_top_t).rotationY(90).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.EAST).with(CapitalBlock.HALF, TopBottomShape.TOP).with(CapitalBlock.TYPE, IntersectionShape.T_LEFT)
                .modelForState().modelFile(capital_top_t).rotationY(180).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.EAST).with(CapitalBlock.HALF, TopBottomShape.TOP).with(CapitalBlock.TYPE, IntersectionShape.T_RIGHT)
                .modelForState().modelFile(capital_top_t).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.EAST).with(CapitalBlock.HALF, TopBottomShape.TOP).with(CapitalBlock.TYPE, IntersectionShape.T_OPPOSITE)
                .modelForState().modelFile(capital_top_t).rotationY(270).uvLock(true).addModel()

                // Bottom
                .partialState().with(CapitalBlock.FACING, Direction.SOUTH).with(CapitalBlock.HALF, TopBottomShape.BOTTOM).with(CapitalBlock.TYPE, IntersectionShape.SINGLE)
                .modelForState().modelFile(capital).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.SOUTH).with(CapitalBlock.HALF, TopBottomShape.BOTTOM).with(CapitalBlock.TYPE, IntersectionShape.END)
                .modelForState().modelFile(capital_end).rotationY(180).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.SOUTH).with(CapitalBlock.HALF, TopBottomShape.BOTTOM).with(CapitalBlock.TYPE, IntersectionShape.CONNECTED)
                .modelForState().modelFile(capital_connected).rotationY(180).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.SOUTH).with(CapitalBlock.HALF, TopBottomShape.BOTTOM).with(CapitalBlock.TYPE, IntersectionShape.INNER_LEFT)
                .modelForState().modelFile(capital_corner).rotationY(180).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.SOUTH).with(CapitalBlock.HALF, TopBottomShape.BOTTOM).with(CapitalBlock.TYPE, IntersectionShape.INNER_RIGHT)
                .modelForState().modelFile(capital_corner).rotationY(90).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.SOUTH).with(CapitalBlock.HALF, TopBottomShape.BOTTOM).with(CapitalBlock.TYPE, IntersectionShape.OUTER_LEFT)
                .modelForState().modelFile(capital_corner).rotationY(270).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.SOUTH).with(CapitalBlock.HALF, TopBottomShape.BOTTOM).with(CapitalBlock.TYPE, IntersectionShape.OUTER_RIGHT)
                .modelForState().modelFile(capital_corner).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.SOUTH).with(CapitalBlock.HALF, TopBottomShape.BOTTOM).with(CapitalBlock.TYPE, IntersectionShape.CROSS)
                .modelForState().modelFile(capital_cross).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.SOUTH).with(CapitalBlock.HALF, TopBottomShape.BOTTOM).with(CapitalBlock.TYPE, IntersectionShape.T)
                .modelForState().modelFile(capital_t).rotationY(180).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.SOUTH).with(CapitalBlock.HALF, TopBottomShape.BOTTOM).with(CapitalBlock.TYPE, IntersectionShape.T_LEFT)
                .modelForState().modelFile(capital_t).rotationY(270).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.SOUTH).with(CapitalBlock.HALF, TopBottomShape.BOTTOM).with(CapitalBlock.TYPE, IntersectionShape.T_RIGHT)
                .modelForState().modelFile(capital_t).rotationY(90).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.SOUTH).with(CapitalBlock.HALF, TopBottomShape.BOTTOM).with(CapitalBlock.TYPE, IntersectionShape.T_OPPOSITE)
                .modelForState().modelFile(capital_top_t).addModel()
                // Top
                .partialState().with(CapitalBlock.FACING, Direction.SOUTH).with(CapitalBlock.HALF, TopBottomShape.TOP).with(CapitalBlock.TYPE, IntersectionShape.SINGLE)
                .modelForState().modelFile(capital_top).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.SOUTH).with(CapitalBlock.HALF, TopBottomShape.TOP).with(CapitalBlock.TYPE, IntersectionShape.END)
                .modelForState().modelFile(capital_top_end).rotationY(180).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.SOUTH).with(CapitalBlock.HALF, TopBottomShape.TOP).with(CapitalBlock.TYPE, IntersectionShape.CONNECTED)
                .modelForState().modelFile(capital_top_connected).rotationY(180).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.SOUTH).with(CapitalBlock.HALF, TopBottomShape.TOP).with(CapitalBlock.TYPE, IntersectionShape.INNER_LEFT)
                .modelForState().modelFile(capital_top_corner).rotationY(180).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.SOUTH).with(CapitalBlock.HALF, TopBottomShape.TOP).with(CapitalBlock.TYPE, IntersectionShape.INNER_RIGHT)
                .modelForState().modelFile(capital_top_corner).rotationY(90).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.SOUTH).with(CapitalBlock.HALF, TopBottomShape.TOP).with(CapitalBlock.TYPE, IntersectionShape.OUTER_LEFT)
                .modelForState().modelFile(capital_top_corner).rotationY(270).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.SOUTH).with(CapitalBlock.HALF, TopBottomShape.TOP).with(CapitalBlock.TYPE, IntersectionShape.OUTER_RIGHT)
                .modelForState().modelFile(capital_top_corner).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.SOUTH).with(CapitalBlock.HALF, TopBottomShape.TOP).with(CapitalBlock.TYPE, IntersectionShape.CROSS)
                .modelForState().modelFile(capital_top_cross).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.SOUTH).with(CapitalBlock.HALF, TopBottomShape.TOP).with(CapitalBlock.TYPE, IntersectionShape.T)
                .modelForState().modelFile(capital_top_t).rotationY(180).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.SOUTH).with(CapitalBlock.HALF, TopBottomShape.TOP).with(CapitalBlock.TYPE, IntersectionShape.T_LEFT)
                .modelForState().modelFile(capital_top_t).rotationY(270).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.SOUTH).with(CapitalBlock.HALF, TopBottomShape.TOP).with(CapitalBlock.TYPE, IntersectionShape.T_RIGHT)
                .modelForState().modelFile(capital_top_t).rotationY(90).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.SOUTH).with(CapitalBlock.HALF, TopBottomShape.TOP).with(CapitalBlock.TYPE, IntersectionShape.T_OPPOSITE)
                .modelForState().modelFile(capital_top_t).addModel()

                // Bottom
                .partialState().with(CapitalBlock.FACING, Direction.WEST).with(CapitalBlock.HALF, TopBottomShape.BOTTOM).with(CapitalBlock.TYPE, IntersectionShape.SINGLE)
                .modelForState().modelFile(capital).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.WEST).with(CapitalBlock.HALF, TopBottomShape.BOTTOM).with(CapitalBlock.TYPE, IntersectionShape.END)
                .modelForState().modelFile(capital_end).rotationY(270).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.WEST).with(CapitalBlock.HALF, TopBottomShape.BOTTOM).with(CapitalBlock.TYPE, IntersectionShape.CONNECTED)
                .modelForState().modelFile(capital_connected).rotationY(270).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.WEST).with(CapitalBlock.HALF, TopBottomShape.BOTTOM).with(CapitalBlock.TYPE, IntersectionShape.INNER_LEFT)
                .modelForState().modelFile(capital_corner).rotationY(270).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.WEST).with(CapitalBlock.HALF, TopBottomShape.BOTTOM).with(CapitalBlock.TYPE, IntersectionShape.INNER_RIGHT)
                .modelForState().modelFile(capital_corner).rotationY(180).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.WEST).with(CapitalBlock.HALF, TopBottomShape.BOTTOM).with(CapitalBlock.TYPE, IntersectionShape.OUTER_LEFT)
                .modelForState().modelFile(capital_corner).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.WEST).with(CapitalBlock.HALF, TopBottomShape.BOTTOM).with(CapitalBlock.TYPE, IntersectionShape.OUTER_RIGHT)
                .modelForState().modelFile(capital_corner).rotationY(90).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.WEST).with(CapitalBlock.HALF, TopBottomShape.BOTTOM).with(CapitalBlock.TYPE, IntersectionShape.CROSS)
                .modelForState().modelFile(capital_cross).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.WEST).with(CapitalBlock.HALF, TopBottomShape.BOTTOM).with(CapitalBlock.TYPE, IntersectionShape.T)
                .modelForState().modelFile(capital_t).rotationY(270).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.WEST).with(CapitalBlock.HALF, TopBottomShape.BOTTOM).with(CapitalBlock.TYPE, IntersectionShape.T_LEFT)
                .modelForState().modelFile(capital_t).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.WEST).with(CapitalBlock.HALF, TopBottomShape.BOTTOM).with(CapitalBlock.TYPE, IntersectionShape.T_RIGHT)
                .modelForState().modelFile(capital_t).rotationY(180).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.WEST).with(CapitalBlock.HALF, TopBottomShape.BOTTOM).with(CapitalBlock.TYPE, IntersectionShape.T_OPPOSITE)
                .modelForState().modelFile(capital_t).rotationY(90).uvLock(true).addModel()
                // Top
                .partialState().with(CapitalBlock.FACING, Direction.WEST).with(CapitalBlock.HALF, TopBottomShape.TOP).with(CapitalBlock.TYPE, IntersectionShape.SINGLE)
                .modelForState().modelFile(capital_top).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.WEST).with(CapitalBlock.HALF, TopBottomShape.TOP).with(CapitalBlock.TYPE, IntersectionShape.END)
                .modelForState().modelFile(capital_top_end).rotationY(270).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.WEST).with(CapitalBlock.HALF, TopBottomShape.TOP).with(CapitalBlock.TYPE, IntersectionShape.CONNECTED)
                .modelForState().modelFile(capital_top_connected).rotationY(270).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.WEST).with(CapitalBlock.HALF, TopBottomShape.TOP).with(CapitalBlock.TYPE, IntersectionShape.INNER_LEFT)
                .modelForState().modelFile(capital_top_corner).rotationY(270).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.WEST).with(CapitalBlock.HALF, TopBottomShape.TOP).with(CapitalBlock.TYPE, IntersectionShape.INNER_RIGHT)
                .modelForState().modelFile(capital_top_corner).rotationY(180).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.WEST).with(CapitalBlock.HALF, TopBottomShape.TOP).with(CapitalBlock.TYPE, IntersectionShape.OUTER_LEFT)
                .modelForState().modelFile(capital_top_corner).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.WEST).with(CapitalBlock.HALF, TopBottomShape.TOP).with(CapitalBlock.TYPE, IntersectionShape.OUTER_RIGHT)
                .modelForState().modelFile(capital_top_corner).rotationY(90).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.WEST).with(CapitalBlock.HALF, TopBottomShape.TOP).with(CapitalBlock.TYPE, IntersectionShape.CROSS)
                .modelForState().modelFile(capital_top_cross).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.WEST).with(CapitalBlock.HALF, TopBottomShape.TOP).with(CapitalBlock.TYPE, IntersectionShape.T)
                .modelForState().modelFile(capital_top_t).rotationY(270).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.WEST).with(CapitalBlock.HALF, TopBottomShape.TOP).with(CapitalBlock.TYPE, IntersectionShape.T_LEFT)
                .modelForState().modelFile(capital_top_t).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.WEST).with(CapitalBlock.HALF, TopBottomShape.TOP).with(CapitalBlock.TYPE, IntersectionShape.T_RIGHT)
                .modelForState().modelFile(capital_top_t).rotationY(180).uvLock(true).addModel()
                .partialState().with(CapitalBlock.FACING, Direction.WEST).with(CapitalBlock.HALF, TopBottomShape.TOP).with(CapitalBlock.TYPE, IntersectionShape.T_OPPOSITE)
                .modelForState().modelFile(capital_top_t).rotationY(90).uvLock(true).addModel();
    }
    ...
    public void doorFrameLintelBlock(DoorFrameLintelBlock block, ResourceLocation side, ResourceLocation bottom, ResourceLocation top) {
        ...
        getVariantBuilder(block)
                ...
                .partialState().with(DoorFrameLintelBlock.FACING, Direction.NORTH).with(DoorFrameLintelBlock.HALF, TopBottomShape.TOP).with(DoorFrameLintelBlock.BEAM, 1)
                .modelForState().modelFile(door_frame_lintel_1).addModel()
                .partialState().with(DoorFrameLintelBlock.FACING, Direction.NORTH).with(DoorFrameLintelBlock.HALF, TopBottomShape.TOP).with(DoorFrameLintelBlock.BEAM, 2)
                .modelForState().modelFile(door_frame_lintel_2).addModel()
                .partialState().with(DoorFrameLintelBlock.FACING, Direction.NORTH).with(DoorFrameLintelBlock.HALF, TopBottomShape.TOP).with(DoorFrameLintelBlock.BEAM, 3)
                .modelForState().modelFile(door_frame_lintel_3).addModel()
                .partialState().with(DoorFrameLintelBlock.FACING, Direction.NORTH).with(DoorFrameLintelBlock.HALF, TopBottomShape.TOP).with(DoorFrameLintelBlock.BEAM, 4)
                .modelForState().modelFile(door_frame_lintel_4).addModel()
                .partialState().with(DoorFrameLintelBlock.FACING, Direction.NORTH).with(DoorFrameLintelBlock.HALF, TopBottomShape.BOTTOM).with(DoorFrameLintelBlock.BEAM, 1)
                .modelForState().modelFile(door_frame_lintel_1).rotationX(180).rotationY(180).addModel()
                .partialState().with(DoorFrameLintelBlock.FACING, Direction.NORTH).with(DoorFrameLintelBlock.HALF, TopBottomShape.BOTTOM).with(DoorFrameLintelBlock.BEAM, 2)
                .modelForState().modelFile(door_frame_lintel_2).rotationX(180).rotationY(180).addModel()
                .partialState().with(DoorFrameLintelBlock.FACING, Direction.NORTH).with(DoorFrameLintelBlock.HALF, TopBottomShape.BOTTOM).with(DoorFrameLintelBlock.BEAM, 3)
                .modelForState().modelFile(door_frame_lintel_3).rotationX(180).rotationY(180).addModel()
                .partialState().with(DoorFrameLintelBlock.FACING, Direction.NORTH).with(DoorFrameLintelBlock.HALF, TopBottomShape.BOTTOM).with(DoorFrameLintelBlock.BEAM, 4)
                .modelForState().modelFile(door_frame_lintel_4).rotationX(180).rotationY(180).addModel()
                .partialState().with(DoorFrameLintelBlock.FACING, Direction.EAST).with(DoorFrameLintelBlock.HALF, TopBottomShape.TOP).with(DoorFrameLintelBlock.BEAM, 1)
                .modelForState().modelFile(door_frame_lintel_1).rotationY(90).addModel()
                .partialState().with(DoorFrameLintelBlock.FACING, Direction.EAST).with(DoorFrameLintelBlock.HALF, TopBottomShape.TOP).with(DoorFrameLintelBlock.BEAM, 2)
                .modelForState().modelFile(door_frame_lintel_2).rotationY(90).addModel()
                .partialState().with(DoorFrameLintelBlock.FACING, Direction.EAST).with(DoorFrameLintelBlock.HALF, TopBottomShape.TOP).with(DoorFrameLintelBlock.BEAM, 3)
                .modelForState().modelFile(door_frame_lintel_3).rotationY(90).addModel()
                .partialState().with(DoorFrameLintelBlock.FACING, Direction.EAST).with(DoorFrameLintelBlock.HALF, TopBottomShape.TOP).with(DoorFrameLintelBlock.BEAM, 4)
                .modelForState().modelFile(door_frame_lintel_4).rotationY(90).addModel()
                .partialState().with(DoorFrameLintelBlock.FACING, Direction.EAST).with(DoorFrameLintelBlock.HALF, TopBottomShape.BOTTOM).with(DoorFrameLintelBlock.BEAM, 1)
                .modelForState().modelFile(door_frame_lintel_1).rotationX(180).rotationY(270).addModel()
                .partialState().with(DoorFrameLintelBlock.FACING, Direction.EAST).with(DoorFrameLintelBlock.HALF, TopBottomShape.BOTTOM).with(DoorFrameLintelBlock.BEAM, 2)
                .modelForState().modelFile(door_frame_lintel_2).rotationX(180).rotationY(270).addModel()
                .partialState().with(DoorFrameLintelBlock.FACING, Direction.EAST).with(DoorFrameLintelBlock.HALF, TopBottomShape.BOTTOM).with(DoorFrameLintelBlock.BEAM, 3)
                .modelForState().modelFile(door_frame_lintel_3).rotationX(180).rotationY(270).addModel()
                .partialState().with(DoorFrameLintelBlock.FACING, Direction.EAST).with(DoorFrameLintelBlock.HALF, TopBottomShape.BOTTOM).with(DoorFrameLintelBlock.BEAM, 4)
                .modelForState().modelFile(door_frame_lintel_4).rotationX(180).rotationY(270).addModel()
                .partialState().with(DoorFrameLintelBlock.FACING, Direction.SOUTH).with(DoorFrameLintelBlock.HALF, TopBottomShape.TOP).with(DoorFrameLintelBlock.BEAM, 1)
                .modelForState().modelFile(door_frame_lintel_1).rotationY(180).addModel()
                .partialState().with(DoorFrameLintelBlock.FACING, Direction.SOUTH).with(DoorFrameLintelBlock.HALF, TopBottomShape.TOP).with(DoorFrameLintelBlock.BEAM, 2)
                .modelForState().modelFile(door_frame_lintel_2).rotationY(180).addModel()
                .partialState().with(DoorFrameLintelBlock.FACING, Direction.SOUTH).with(DoorFrameLintelBlock.HALF, TopBottomShape.TOP).with(DoorFrameLintelBlock.BEAM, 3)
                .modelForState().modelFile(door_frame_lintel_3).rotationY(180).addModel()
                .partialState().with(DoorFrameLintelBlock.FACING, Direction.SOUTH).with(DoorFrameLintelBlock.HALF, TopBottomShape.TOP).with(DoorFrameLintelBlock.BEAM, 4)
                .modelForState().modelFile(door_frame_lintel_4).rotationY(180).addModel()
                .partialState().with(DoorFrameLintelBlock.FACING, Direction.SOUTH).with(DoorFrameLintelBlock.HALF, TopBottomShape.BOTTOM).with(DoorFrameLintelBlock.BEAM, 1)
                .modelForState().modelFile(door_frame_lintel_1).rotationX(180).addModel()
                .partialState().with(DoorFrameLintelBlock.FACING, Direction.SOUTH).with(DoorFrameLintelBlock.HALF, TopBottomShape.BOTTOM).with(DoorFrameLintelBlock.BEAM, 2)
                .modelForState().modelFile(door_frame_lintel_2).rotationX(180).addModel()
                .partialState().with(DoorFrameLintelBlock.FACING, Direction.SOUTH).with(DoorFrameLintelBlock.HALF, TopBottomShape.BOTTOM).with(DoorFrameLintelBlock.BEAM, 3)
                .modelForState().modelFile(door_frame_lintel_3).rotationX(180).addModel()
                .partialState().with(DoorFrameLintelBlock.FACING, Direction.SOUTH).with(DoorFrameLintelBlock.HALF, TopBottomShape.BOTTOM).with(DoorFrameLintelBlock.BEAM, 4)
                .modelForState().modelFile(door_frame_lintel_4).rotationX(180).addModel()
                .partialState().with(DoorFrameLintelBlock.FACING, Direction.WEST).with(DoorFrameLintelBlock.HALF, TopBottomShape.TOP).with(DoorFrameLintelBlock.BEAM, 1)
                .modelForState().modelFile(door_frame_lintel_1).rotationY(270).addModel()
                .partialState().with(DoorFrameLintelBlock.FACING, Direction.WEST).with(DoorFrameLintelBlock.HALF, TopBottomShape.TOP).with(DoorFrameLintelBlock.BEAM, 2)
                .modelForState().modelFile(door_frame_lintel_2).rotationY(270).addModel()
                .partialState().with(DoorFrameLintelBlock.FACING, Direction.WEST).with(DoorFrameLintelBlock.HALF, TopBottomShape.TOP).with(DoorFrameLintelBlock.BEAM, 3)
                .modelForState().modelFile(door_frame_lintel_3).rotationY(270).addModel()
                .partialState().with(DoorFrameLintelBlock.FACING, Direction.WEST).with(DoorFrameLintelBlock.HALF, TopBottomShape.TOP).with(DoorFrameLintelBlock.BEAM, 4)
                .modelForState().modelFile(door_frame_lintel_4).rotationY(270).addModel()
                .partialState().with(DoorFrameLintelBlock.FACING, Direction.WEST).with(DoorFrameLintelBlock.HALF, TopBottomShape.BOTTOM).with(DoorFrameLintelBlock.BEAM, 1)
                .modelForState().modelFile(door_frame_lintel_1).rotationX(180).rotationY(90).addModel()
                .partialState().with(DoorFrameLintelBlock.FACING, Direction.WEST).with(DoorFrameLintelBlock.HALF, TopBottomShape.BOTTOM).with(DoorFrameLintelBlock.BEAM, 2)
                .modelForState().modelFile(door_frame_lintel_2).rotationX(180).rotationY(90).addModel()
                .partialState().with(DoorFrameLintelBlock.FACING, Direction.WEST).with(DoorFrameLintelBlock.HALF, TopBottomShape.BOTTOM).with(DoorFrameLintelBlock.BEAM, 3)
                .modelForState().modelFile(door_frame_lintel_3).rotationX(180).rotationY(90).addModel()
                .partialState().with(DoorFrameLintelBlock.FACING, Direction.WEST).with(DoorFrameLintelBlock.HALF, TopBottomShape.BOTTOM).with(DoorFrameLintelBlock.BEAM, 4)
    ...
    public void quarterLayerBlock(QuarterLayerBlock block, ResourceLocation side, ResourceLocation bottom, ResourceLocation top, ResourceLocation baseBlock) {
        ...
        getVariantBuilder(block)
                .partialState().with(QuarterLayerBlock.FACING, Direction.NORTH).with(QuarterLayerBlock.HALF, TopBottomShape.BOTTOM).with(QuarterLayerBlock.LAYERS, 1)
                .modelForState().modelFile(quarter_layer_1).addModel()
                .partialState().with(QuarterLayerBlock.FACING, Direction.NORTH).with(QuarterLayerBlock.HALF, TopBottomShape.BOTTOM).with(QuarterLayerBlock.LAYERS, 2)
                .modelForState().modelFile(quarter_layer_2).addModel()
                .partialState().with(QuarterLayerBlock.FACING, Direction.NORTH).with(QuarterLayerBlock.HALF, TopBottomShape.BOTTOM).with(QuarterLayerBlock.LAYERS, 3)
                .modelForState().modelFile(quarter_layer_3).addModel()
                .partialState().with(QuarterLayerBlock.FACING, Direction.NORTH).with(QuarterLayerBlock.HALF, TopBottomShape.BOTTOM).with(QuarterLayerBlock.LAYERS, 4)
                .modelForState().modelFile(quarter_layer_4).addModel()
                .partialState().with(QuarterLayerBlock.FACING, Direction.NORTH).with(QuarterLayerBlock.HALF, TopBottomShape.BOTTOM).with(QuarterLayerBlock.LAYERS, 5)
                .modelForState().modelFile(full_block).addModel()
                .partialState().with(QuarterLayerBlock.FACING, Direction.NORTH).with(QuarterLayerBlock.HALF, TopBottomShape.TOP).with(QuarterLayerBlock.LAYERS, 1)
                .modelForState().modelFile(quarter_top_layer_1).addModel()
                .partialState().with(QuarterLayerBlock.FACING, Direction.NORTH).with(QuarterLayerBlock.HALF, TopBottomShape.TOP).with(QuarterLayerBlock.LAYERS, 2)
                .modelForState().modelFile(quarter_top_layer_2).addModel()
                .partialState().with(QuarterLayerBlock.FACING, Direction.NORTH).with(QuarterLayerBlock.HALF, TopBottomShape.TOP).with(QuarterLayerBlock.LAYERS, 3)
                .modelForState().modelFile(quarter_top_layer_3).addModel()
                .partialState().with(QuarterLayerBlock.FACING, Direction.NORTH).with(QuarterLayerBlock.HALF, TopBottomShape.TOP).with(QuarterLayerBlock.LAYERS, 4)
                .modelForState().modelFile(quarter_top_layer_4).addModel()
                .partialState().with(QuarterLayerBlock.FACING, Direction.NORTH).with(QuarterLayerBlock.HALF, TopBottomShape.TOP).with(QuarterLayerBlock.LAYERS, 5)
                .modelForState().modelFile(full_block).addModel()
                .partialState().with(QuarterLayerBlock.FACING, Direction.EAST).with(QuarterLayerBlock.HALF, TopBottomShape.BOTTOM).with(QuarterLayerBlock.LAYERS, 1)
                .modelForState().modelFile(quarter_layer_1).rotationY(90).addModel()
                .partialState().with(QuarterLayerBlock.FACING, Direction.EAST).with(QuarterLayerBlock.HALF, TopBottomShape.BOTTOM).with(QuarterLayerBlock.LAYERS, 2)
                .modelForState().modelFile(quarter_layer_2).rotationY(90).addModel()
                .partialState().with(QuarterLayerBlock.FACING, Direction.EAST).with(QuarterLayerBlock.HALF, TopBottomShape.BOTTOM).with(QuarterLayerBlock.LAYERS, 3)
                .modelForState().modelFile(quarter_layer_3).rotationY(90).addModel()
                .partialState().with(QuarterLayerBlock.FACING, Direction.EAST).with(QuarterLayerBlock.HALF, TopBottomShape.BOTTOM).with(QuarterLayerBlock.LAYERS, 4)
                .modelForState().modelFile(quarter_layer_4).rotationY(90).addModel()
                .partialState().with(QuarterLayerBlock.FACING, Direction.EAST).with(QuarterLayerBlock.HALF, TopBottomShape.BOTTOM).with(QuarterLayerBlock.LAYERS, 5)
                .modelForState().modelFile(full_block).rotationY(90).addModel()
                .partialState().with(QuarterLayerBlock.FACING, Direction.EAST).with(QuarterLayerBlock.HALF, TopBottomShape.TOP).with(QuarterLayerBlock.LAYERS, 1)
                .modelForState().modelFile(quarter_top_layer_1).rotationY(90).addModel()
                .partialState().with(QuarterLayerBlock.FACING, Direction.EAST).with(QuarterLayerBlock.HALF, TopBottomShape.TOP).with(QuarterLayerBlock.LAYERS, 2)
                .modelForState().modelFile(quarter_top_layer_2).rotationY(90).addModel()
                .partialState().with(QuarterLayerBlock.FACING, Direction.EAST).with(QuarterLayerBlock.HALF, TopBottomShape.TOP).with(QuarterLayerBlock.LAYERS, 3)
                .modelForState().modelFile(quarter_top_layer_3).rotationY(90).addModel()
                .partialState().with(QuarterLayerBlock.FACING, Direction.EAST).with(QuarterLayerBlock.HALF, TopBottomShape.TOP).with(QuarterLayerBlock.LAYERS, 4)
                .modelForState().modelFile(quarter_top_layer_4).rotationY(90).addModel()
                .partialState().with(QuarterLayerBlock.FACING, Direction.EAST).with(QuarterLayerBlock.HALF, TopBottomShape.TOP).with(QuarterLayerBlock.LAYERS, 5)
                .modelForState().modelFile(full_block).rotationY(90).addModel()
                .partialState().with(QuarterLayerBlock.FACING, Direction.SOUTH).with(QuarterLayerBlock.HALF, TopBottomShape.BOTTOM).with(QuarterLayerBlock.LAYERS, 1)
                .modelForState().modelFile(quarter_layer_1).rotationY(180).addModel()
                .partialState().with(QuarterLayerBlock.FACING, Direction.SOUTH).with(QuarterLayerBlock.HALF, TopBottomShape.BOTTOM).with(QuarterLayerBlock.LAYERS, 2)
                .modelForState().modelFile(quarter_layer_2).rotationY(180).addModel()
                .partialState().with(QuarterLayerBlock.FACING, Direction.SOUTH).with(QuarterLayerBlock.HALF, TopBottomShape.BOTTOM).with(QuarterLayerBlock.LAYERS, 3)
                .modelForState().modelFile(quarter_layer_3).rotationY(180).addModel()
                .partialState().with(QuarterLayerBlock.FACING, Direction.SOUTH).with(QuarterLayerBlock.HALF, TopBottomShape.BOTTOM).with(QuarterLayerBlock.LAYERS, 4)
                .modelForState().modelFile(quarter_layer_4).rotationY(180).addModel()
                .partialState().with(QuarterLayerBlock.FACING, Direction.SOUTH).with(QuarterLayerBlock.HALF, TopBottomShape.BOTTOM).with(QuarterLayerBlock.LAYERS, 5)
                .modelForState().modelFile(full_block).rotationY(180).addModel()
                .partialState().with(QuarterLayerBlock.FACING, Direction.SOUTH).with(QuarterLayerBlock.HALF, TopBottomShape.TOP).with(QuarterLayerBlock.LAYERS, 1)
                .modelForState().modelFile(quarter_top_layer_1).rotationY(180).addModel()
                .partialState().with(QuarterLayerBlock.FACING, Direction.SOUTH).with(QuarterLayerBlock.HALF, TopBottomShape.TOP).with(QuarterLayerBlock.LAYERS, 2)
                .modelForState().modelFile(quarter_top_layer_2).rotationY(180).addModel()
                .partialState().with(QuarterLayerBlock.FACING, Direction.SOUTH).with(QuarterLayerBlock.HALF, TopBottomShape.TOP).with(QuarterLayerBlock.LAYERS, 3)
                .modelForState().modelFile(quarter_top_layer_3).rotationY(180).addModel()
                .partialState().with(QuarterLayerBlock.FACING, Direction.SOUTH).with(QuarterLayerBlock.HALF, TopBottomShape.TOP).with(QuarterLayerBlock.LAYERS, 4)
                .modelForState().modelFile(quarter_top_layer_4).rotationY(180).addModel()
                .partialState().with(QuarterLayerBlock.FACING, Direction.SOUTH).with(QuarterLayerBlock.HALF, TopBottomShape.TOP).with(QuarterLayerBlock.LAYERS, 5)
                .modelForState().modelFile(full_block).rotationY(180).addModel()
                .partialState().with(QuarterLayerBlock.FACING, Direction.WEST).with(QuarterLayerBlock.HALF, TopBottomShape.BOTTOM).with(QuarterLayerBlock.LAYERS, 1)
                .modelForState().modelFile(quarter_layer_1).rotationY(270).addModel()
                .partialState().with(QuarterLayerBlock.FACING, Direction.WEST).with(QuarterLayerBlock.HALF, TopBottomShape.BOTTOM).with(QuarterLayerBlock.LAYERS, 2)
                .modelForState().modelFile(quarter_layer_2).rotationY(270).addModel()
                .partialState().with(QuarterLayerBlock.FACING, Direction.WEST).with(QuarterLayerBlock.HALF, TopBottomShape.BOTTOM).with(QuarterLayerBlock.LAYERS, 3)
                .modelForState().modelFile(quarter_layer_3).rotationY(270).addModel()
                .partialState().with(QuarterLayerBlock.FACING, Direction.WEST).with(QuarterLayerBlock.HALF, TopBottomShape.BOTTOM).with(QuarterLayerBlock.LAYERS, 4)
                .modelForState().modelFile(quarter_layer_4).rotationY(270).addModel()
                .partialState().with(QuarterLayerBlock.FACING, Direction.WEST).with(QuarterLayerBlock.HALF, TopBottomShape.BOTTOM).with(QuarterLayerBlock.LAYERS, 5)
                .modelForState().modelFile(full_block).rotationY(270).addModel()
                .partialState().with(QuarterLayerBlock.FACING, Direction.WEST).with(QuarterLayerBlock.HALF, TopBottomShape.TOP).with(QuarterLayerBlock.LAYERS, 1)
                .modelForState().modelFile(quarter_top_layer_1).rotationY(270).addModel()
                .partialState().with(QuarterLayerBlock.FACING, Direction.WEST).with(QuarterLayerBlock.HALF, TopBottomShape.TOP).with(QuarterLayerBlock.LAYERS, 2)
                .modelForState().modelFile(quarter_top_layer_2).rotationY(270).addModel()
                .partialState().with(QuarterLayerBlock.FACING, Direction.WEST).with(QuarterLayerBlock.HALF, TopBottomShape.TOP).with(QuarterLayerBlock.LAYERS, 3)
                .modelForState().modelFile(quarter_top_layer_3).rotationY(270).addModel()
                .partialState().with(QuarterLayerBlock.FACING, Direction.WEST).with(QuarterLayerBlock.HALF, TopBottomShape.TOP).with(QuarterLayerBlock.LAYERS, 4)
                .modelForState().modelFile(quarter_top_layer_4).rotationY(270).addModel()
                .partialState().with(QuarterLayerBlock.FACING, Direction.WEST).with(QuarterLayerBlock.HALF, TopBottomShape.TOP).with(QuarterLayerBlock.LAYERS, 5)
                .modelForState().modelFile(full_block).rotationY(270).addModel();
    }
    ...
    public void slabLayerBlock(SlabLayerBlock block, ResourceLocation side, ResourceLocation bottom, ResourceLocation top, ResourceLocation baseBlock) {
        ...
        getVariantBuilder(block)
                .partialState().with(SlabLayerBlock.HALF, TopBottomShape.BOTTOM).with(SlabLayerBlock.LAYERS, 1)
                .modelForState().modelFile(slab_layer_1).addModel()
                .partialState().with(SlabLayerBlock.HALF, TopBottomShape.BOTTOM).with(SlabLayerBlock.LAYERS, 2)
                .modelForState().modelFile(slab_layer_2).addModel()
                .partialState().with(SlabLayerBlock.HALF, TopBottomShape.BOTTOM).with(SlabLayerBlock.LAYERS, 3)
                .modelForState().modelFile(slab_layer_3).addModel()
                .partialState().with(SlabLayerBlock.HALF, TopBottomShape.BOTTOM).with(SlabLayerBlock.LAYERS, 4)
                .modelForState().modelFile(slab_layer_4).addModel()
                .partialState().with(SlabLayerBlock.HALF, TopBottomShape.BOTTOM).with(SlabLayerBlock.LAYERS, 5)
                .modelForState().modelFile(slab_layer_5).addModel()
                .partialState().with(SlabLayerBlock.HALF, TopBottomShape.BOTTOM).with(SlabLayerBlock.LAYERS, 6)
                .modelForState().modelFile(slab_layer_6).addModel()
                .partialState().with(SlabLayerBlock.HALF, TopBottomShape.BOTTOM).with(SlabLayerBlock.LAYERS, 7)
                .modelForState().modelFile(slab_layer_7).addModel()
                .partialState().with(SlabLayerBlock.HALF, TopBottomShape.BOTTOM).with(SlabLayerBlock.LAYERS, 8)
                .modelForState().modelFile(full_block).addModel()
                .partialState().with(SlabLayerBlock.HALF, TopBottomShape.TOP).with(SlabLayerBlock.LAYERS, 1)
                .modelForState().modelFile(slab_layer_1).rotationX(180).uvLock(true).addModel()
                .partialState().with(SlabLayerBlock.HALF, TopBottomShape.TOP).with(SlabLayerBlock.LAYERS, 2)
                .modelForState().modelFile(slab_layer_2).rotationX(180).uvLock(true).addModel()
                .partialState().with(SlabLayerBlock.HALF, TopBottomShape.TOP).with(SlabLayerBlock.LAYERS, 3)
                .modelForState().modelFile(slab_layer_3).rotationX(180).uvLock(true).addModel()
                .partialState().with(SlabLayerBlock.HALF, TopBottomShape.TOP).with(SlabLayerBlock.LAYERS, 4)
                .modelForState().modelFile(slab_layer_4).rotationX(180).uvLock(true).addModel()
                .partialState().with(SlabLayerBlock.HALF, TopBottomShape.TOP).with(SlabLayerBlock.LAYERS, 5)
                .modelForState().modelFile(slab_layer_5).rotationX(180).uvLock(true).addModel()
                .partialState().with(SlabLayerBlock.HALF, TopBottomShape.TOP).with(SlabLayerBlock.LAYERS, 6)
                .modelForState().modelFile(slab_layer_6).rotationX(180).uvLock(true).addModel()
                .partialState().with(SlabLayerBlock.HALF, TopBottomShape.TOP).with(SlabLayerBlock.LAYERS, 7)
                .modelForState().modelFile(slab_layer_7).rotationX(180).uvLock(true).addModel()
                .partialState().with(SlabLayerBlock.HALF, TopBottomShape.TOP).with(SlabLayerBlock.LAYERS, 8)
                .modelForState().modelFile(full_block).addModel();
    }
    ...
    public void slabVerticalBlock(VerticalSlabBlock block) {
        slabVerticalBlock(block, blockTexture(block));
    }

    public void slabVerticalBlock(VerticalSlabBlock block, ResourceLocation texture) {
        slabVerticalBlock(block, texture, texture, texture, texture);
    }

    public void slabVerticalBlock(VerticalSlabBlock block, ResourceLocation side, ResourceLocation bottom, ResourceLocation top, ResourceLocation baseBlock) {
        ModelFile slab_vertical = models().withExistingParent(name(block), modLoc("block/templates/slab_vertical"))
                .texture("side", side)
                .texture("top", top)
                .texture("bottom", bottom),
                full_block = this.models().getExistingFile(baseBlock);;
        ...
        getVariantBuilder(block)
                .partialState().with(VerticalSlabBlock.FACING, Direction.NORTH).with(VerticalSlabBlock.TYPE, SingleDoubleShape.SINGLE)
                .modelForState().modelFile(slab_vertical).addModel()
                .partialState().with(VerticalSlabBlock.FACING, Direction.NORTH).with(VerticalSlabBlock.TYPE, SingleDoubleShape.DOUBLE)
                .modelForState().modelFile(full_block).addModel()
                .partialState().with(VerticalSlabBlock.FACING, Direction.EAST).with(VerticalSlabBlock.TYPE, SingleDoubleShape.SINGLE)
                .modelForState().modelFile(slab_vertical).addModel()
                .partialState().with(VerticalSlabBlock.FACING, Direction.EAST).with(VerticalSlabBlock.TYPE, SingleDoubleShape.DOUBLE)
                .modelForState().modelFile(full_block).addModel()
                .partialState().with(VerticalSlabBlock.FACING, Direction.SOUTH).with(VerticalSlabBlock.TYPE, SingleDoubleShape.SINGLE)
                .modelForState().modelFile(slab_vertical).addModel()
                .partialState().with(VerticalSlabBlock.FACING, Direction.SOUTH).with(VerticalSlabBlock.TYPE, SingleDoubleShape.DOUBLE)
                .modelForState().modelFile(full_block).addModel()
                .partialState().with(VerticalSlabBlock.FACING, Direction.WEST).with(VerticalSlabBlock.TYPE, SingleDoubleShape.SINGLE)
                .modelForState().modelFile(slab_vertical).addModel()
                .partialState().with(VerticalSlabBlock.FACING, Direction.WEST).with(VerticalSlabBlock.TYPE, SingleDoubleShape.DOUBLE)
                .modelForState().modelFile(full_block).addModel();
    }
    ...
    public void windowBlock(WindowBlock block, ResourceLocation side, ResourceLocation bottom, ResourceLocation top) {
        ModelFile window = models().withExistingParent(name(block), modLoc("block/templates/window"))
                ...
                .texture("top", top)
                .texture("bottom", bottom),
                window_arch = models().withExistingParent(name(block) + "_arch", modLoc("block/templates/window_arch"))
                        .texture("side", side)
                        .texture("top", top)
                        .texture("bottom", bottom),
                window_top = models().withExistingParent(name(block) + "_top", modLoc("block/templates/window_top"))
                        ...
                        .texture("top", top)
                        .texture("bottom", bottom),
                window_arch_top = models().withExistingParent(name(block) + "_arch_top", modLoc("block/templates/window_arch_top"))
                        .texture("side", side)
                        .texture("top", top)
                        .texture("bottom", bottom),
        ...
        getVariantBuilder(block)
                .partialState().with(WindowBlock.FACING, Direction.NORTH).with(WindowBlock.TYPE, WindowShape.TOP).with(WindowBlock.ARCH,false)
                .modelForState().modelFile(window_top).addModel()
                .partialState().with(WindowBlock.FACING, Direction.NORTH).with(WindowBlock.TYPE, WindowShape.TOP).with(WindowBlock.ARCH,true)
                .modelForState().modelFile(window_arch_top).addModel()
                .partialState().with(WindowBlock.FACING, Direction.NORTH).with(WindowBlock.TYPE, WindowShape.MIDDLE).with(WindowBlock.ARCH,false)
                .modelForState().modelFile(window_middle).addModel()
                .partialState().with(WindowBlock.FACING, Direction.NORTH).with(WindowBlock.TYPE, WindowShape.MIDDLE).with(WindowBlock.ARCH,true)
                .modelForState().modelFile(window_middle).addModel()
                .partialState().with(WindowBlock.FACING, Direction.NORTH).with(WindowBlock.TYPE, WindowShape.BOTTOM).with(WindowBlock.ARCH,false)
                .modelForState().modelFile(window_bottom).addModel()
                .partialState().with(WindowBlock.FACING, Direction.NORTH).with(WindowBlock.TYPE, WindowShape.BOTTOM).with(WindowBlock.ARCH,true)
                .modelForState().modelFile(window_bottom).addModel()
                .partialState().with(WindowBlock.FACING, Direction.NORTH).with(WindowBlock.TYPE, WindowShape.FULL_BLOCK).with(WindowBlock.ARCH,false)
                .modelForState().modelFile(window).addModel()
                .partialState().with(WindowBlock.FACING, Direction.NORTH).with(WindowBlock.TYPE, WindowShape.FULL_BLOCK).with(WindowBlock.ARCH,true)
                .modelForState().modelFile(window_arch).addModel()
                .partialState().with(WindowBlock.FACING, Direction.EAST).with(WindowBlock.TYPE, WindowShape.TOP).with(WindowBlock.ARCH,false)
                .modelForState().modelFile(window_top).rotationY(90).addModel()
                .partialState().with(WindowBlock.FACING, Direction.EAST).with(WindowBlock.TYPE, WindowShape.TOP).with(WindowBlock.ARCH,true)
                .modelForState().modelFile(window_arch_top).rotationY(90).addModel()
                .partialState().with(WindowBlock.FACING, Direction.EAST).with(WindowBlock.TYPE, WindowShape.MIDDLE).with(WindowBlock.ARCH,false)
                .modelForState().modelFile(window_middle).rotationY(90).addModel()
                .partialState().with(WindowBlock.FACING, Direction.EAST).with(WindowBlock.TYPE, WindowShape.MIDDLE).with(WindowBlock.ARCH,true)
                .modelForState().modelFile(window_middle).rotationY(90).addModel()
                .partialState().with(WindowBlock.FACING, Direction.EAST).with(WindowBlock.TYPE, WindowShape.BOTTOM).with(WindowBlock.ARCH,false)
                .modelForState().modelFile(window_bottom).rotationY(90).addModel()
                .partialState().with(WindowBlock.FACING, Direction.EAST).with(WindowBlock.TYPE, WindowShape.BOTTOM).with(WindowBlock.ARCH,true)
                .modelForState().modelFile(window_bottom).rotationY(90).addModel()
                .partialState().with(WindowBlock.FACING, Direction.EAST).with(WindowBlock.TYPE, WindowShape.FULL_BLOCK).with(WindowBlock.ARCH,false)
                .modelForState().modelFile(window).rotationY(90).addModel()
                .partialState().with(WindowBlock.FACING, Direction.EAST).with(WindowBlock.TYPE, WindowShape.FULL_BLOCK).with(WindowBlock.ARCH,true)
                .modelForState().modelFile(window_arch).rotationY(90).addModel()
                .partialState().with(WindowBlock.FACING, Direction.SOUTH).with(WindowBlock.TYPE, WindowShape.TOP).with(WindowBlock.ARCH,false)
                .modelForState().modelFile(window_top).rotationY(180).addModel()
                .partialState().with(WindowBlock.FACING, Direction.SOUTH).with(WindowBlock.TYPE, WindowShape.TOP).with(WindowBlock.ARCH,true)
                .modelForState().modelFile(window_arch_top).rotationY(180).addModel()
                .partialState().with(WindowBlock.FACING, Direction.SOUTH).with(WindowBlock.TYPE, WindowShape.MIDDLE).with(WindowBlock.ARCH,false)
                .modelForState().modelFile(window_middle).rotationY(180).addModel()
                .partialState().with(WindowBlock.FACING, Direction.SOUTH).with(WindowBlock.TYPE, WindowShape.MIDDLE).with(WindowBlock.ARCH,true)
                .modelForState().modelFile(window_middle).rotationY(180).addModel()
                .partialState().with(WindowBlock.FACING, Direction.SOUTH).with(WindowBlock.TYPE, WindowShape.BOTTOM).with(WindowBlock.ARCH,false)
                .modelForState().modelFile(window_bottom).rotationY(180).addModel()
                .partialState().with(WindowBlock.FACING, Direction.SOUTH).with(WindowBlock.TYPE, WindowShape.BOTTOM).with(WindowBlock.ARCH,true)
                .modelForState().modelFile(window_bottom).rotationY(180).addModel()
                .partialState().with(WindowBlock.FACING, Direction.SOUTH).with(WindowBlock.TYPE, WindowShape.FULL_BLOCK).with(WindowBlock.ARCH,false)
                .modelForState().modelFile(window).rotationY(180).addModel()
                .partialState().with(WindowBlock.FACING, Direction.SOUTH).with(WindowBlock.TYPE, WindowShape.FULL_BLOCK).with(WindowBlock.ARCH,true)
                .modelForState().modelFile(window_arch).rotationY(180).addModel()
                .partialState().with(WindowBlock.FACING, Direction.WEST).with(WindowBlock.TYPE, WindowShape.TOP).with(WindowBlock.ARCH,false)
                .modelForState().modelFile(window_top).rotationY(270).addModel()
                .partialState().with(WindowBlock.FACING, Direction.WEST).with(WindowBlock.TYPE, WindowShape.TOP).with(WindowBlock.ARCH,true)
                .modelForState().modelFile(window_arch_top).rotationY(270).addModel()
                .partialState().with(WindowBlock.FACING, Direction.WEST).with(WindowBlock.TYPE, WindowShape.MIDDLE).with(WindowBlock.ARCH,false)
                .modelForState().modelFile(window_middle).rotationY(270).addModel()
                .partialState().with(WindowBlock.FACING, Direction.WEST).with(WindowBlock.TYPE, WindowShape.MIDDLE).with(WindowBlock.ARCH,true)
                .modelForState().modelFile(window_middle).rotationY(270).addModel()
                .partialState().with(WindowBlock.FACING, Direction.WEST).with(WindowBlock.TYPE, WindowShape.BOTTOM).with(WindowBlock.ARCH,false)
                .modelForState().modelFile(window_bottom).rotationY(270).addModel()
                .partialState().with(WindowBlock.FACING, Direction.WEST).with(WindowBlock.TYPE, WindowShape.BOTTOM).with(WindowBlock.ARCH,true)
                .modelForState().modelFile(window_bottom).rotationY(270).addModel()
                .partialState().with(WindowBlock.FACING, Direction.WEST).with(WindowBlock.TYPE, WindowShape.FULL_BLOCK).with(WindowBlock.ARCH,false)
                .modelForState().modelFile(window).rotationY(270).addModel()
                .partialState().with(WindowBlock.FACING, Direction.WEST).with(WindowBlock.TYPE, WindowShape.FULL_BLOCK).with(WindowBlock.ARCH,true)
                .modelForState().modelFile(window_arch).rotationY(270).addModel();
    }
    ...
    public void windowHalfBlock(HalfWindowBlock block, ResourceLocation side, ResourceLocation bottom, ResourceLocation top) {
        ModelFile window_half = models().withExistingParent(name(block), modLoc("block/templates/window_half"))
                ...
                .texture("top", top)
                .texture("bottom", bottom),
                window_half_arch = models().withExistingParent(name(block) + "_arch", modLoc("block/templates/window_half_arch"))
                        .texture("side", side)
                        .texture("top", top)
                        .texture("bottom", bottom),
                window_half_top = models().withExistingParent(name(block) + "_trim", modLoc("block/templates/window_half_top"))
                        ...
                        .texture("top", top)
                        .texture("bottom", bottom),
                window_half_arch_top = models().withExistingParent(name(block) + "_arch_top", modLoc("block/templates/window_half_arch_top"))
                        .texture("side", side)
                        .texture("top", top)
                        .texture("bottom", bottom),
        ...
        getVariantBuilder(block)
                .partialState().with(HalfWindowBlock.FACING, Direction.NORTH).with(HalfWindowBlock.TYPE, WindowShape.TOP).with(HalfWindowBlock.ARCH,false)
                .modelForState().modelFile(window_half_top).addModel()
                .partialState().with(HalfWindowBlock.FACING, Direction.NORTH).with(HalfWindowBlock.TYPE, WindowShape.TOP).with(HalfWindowBlock.ARCH,true)
                .modelForState().modelFile(window_half_arch_top).addModel()
                .partialState().with(HalfWindowBlock.FACING, Direction.NORTH).with(HalfWindowBlock.TYPE, WindowShape.MIDDLE).with(HalfWindowBlock.ARCH,false)
                .modelForState().modelFile(window_half_middle).addModel()
                .partialState().with(HalfWindowBlock.FACING, Direction.NORTH).with(HalfWindowBlock.TYPE, WindowShape.MIDDLE).with(HalfWindowBlock.ARCH,true)
                .modelForState().modelFile(window_half_middle).addModel()
                .partialState().with(HalfWindowBlock.FACING, Direction.NORTH).with(HalfWindowBlock.TYPE, WindowShape.BOTTOM).with(HalfWindowBlock.ARCH,false)
                .modelForState().modelFile(window_half_bottom).addModel()
                .partialState().with(HalfWindowBlock.FACING, Direction.NORTH).with(HalfWindowBlock.TYPE, WindowShape.BOTTOM).with(HalfWindowBlock.ARCH,true)
                .modelForState().modelFile(window_half_bottom).addModel()
                .partialState().with(HalfWindowBlock.FACING, Direction.NORTH).with(HalfWindowBlock.TYPE, WindowShape.FULL_BLOCK).with(HalfWindowBlock.ARCH,false)
                .modelForState().modelFile(window_half).addModel()
                .partialState().with(HalfWindowBlock.FACING, Direction.NORTH).with(HalfWindowBlock.TYPE, WindowShape.FULL_BLOCK).with(HalfWindowBlock.ARCH,true)
                .modelForState().modelFile(window_half_arch).addModel()
                .partialState().with(HalfWindowBlock.FACING, Direction.EAST).with(HalfWindowBlock.TYPE, WindowShape.TOP).with(HalfWindowBlock.ARCH,false)
                .modelForState().modelFile(window_half_top).rotationY(90).addModel()
                .partialState().with(HalfWindowBlock.FACING, Direction.EAST).with(HalfWindowBlock.TYPE, WindowShape.TOP).with(HalfWindowBlock.ARCH,true)
                .modelForState().modelFile(window_half_arch_top).rotationY(90).addModel()
                .partialState().with(HalfWindowBlock.FACING, Direction.EAST).with(HalfWindowBlock.TYPE, WindowShape.MIDDLE).with(HalfWindowBlock.ARCH,false)
                .modelForState().modelFile(window_half_middle).rotationY(90).addModel()
                .partialState().with(HalfWindowBlock.FACING, Direction.EAST).with(HalfWindowBlock.TYPE, WindowShape.MIDDLE).with(HalfWindowBlock.ARCH,true)
                .modelForState().modelFile(window_half_middle).rotationY(90).addModel()
                .partialState().with(HalfWindowBlock.FACING, Direction.EAST).with(HalfWindowBlock.TYPE, WindowShape.BOTTOM).with(HalfWindowBlock.ARCH,false)
                .modelForState().modelFile(window_half_bottom).rotationY(90).addModel()
                .partialState().with(HalfWindowBlock.FACING, Direction.EAST).with(HalfWindowBlock.TYPE, WindowShape.BOTTOM).with(HalfWindowBlock.ARCH,true)
                .modelForState().modelFile(window_half_bottom).rotationY(90).addModel()
                .partialState().with(HalfWindowBlock.FACING, Direction.EAST).with(HalfWindowBlock.TYPE, WindowShape.FULL_BLOCK).with(HalfWindowBlock.ARCH,false)
                .modelForState().modelFile(window_half).rotationY(90).addModel()
                .partialState().with(HalfWindowBlock.FACING, Direction.EAST).with(HalfWindowBlock.TYPE, WindowShape.FULL_BLOCK).with(HalfWindowBlock.ARCH,true)
                .modelForState().modelFile(window_half_arch).rotationY(90).addModel()
                .partialState().with(HalfWindowBlock.FACING, Direction.SOUTH).with(HalfWindowBlock.TYPE, WindowShape.TOP).with(HalfWindowBlock.ARCH,false)
                .modelForState().modelFile(window_half_top).rotationY(180).addModel()
                .partialState().with(HalfWindowBlock.FACING, Direction.SOUTH).with(HalfWindowBlock.TYPE, WindowShape.TOP).with(HalfWindowBlock.ARCH,true)
                .modelForState().modelFile(window_half_arch_top).rotationY(180).addModel()
                .partialState().with(HalfWindowBlock.FACING, Direction.SOUTH).with(HalfWindowBlock.TYPE, WindowShape.MIDDLE).with(HalfWindowBlock.ARCH,false)
                .modelForState().modelFile(window_half_middle).rotationY(180).addModel()
                .partialState().with(HalfWindowBlock.FACING, Direction.SOUTH).with(HalfWindowBlock.TYPE, WindowShape.MIDDLE).with(HalfWindowBlock.ARCH,true)
                .modelForState().modelFile(window_half_middle).rotationY(180).addModel()
                .partialState().with(HalfWindowBlock.FACING, Direction.SOUTH).with(HalfWindowBlock.TYPE, WindowShape.BOTTOM).with(HalfWindowBlock.ARCH,false)
                .modelForState().modelFile(window_half_bottom).rotationY(180).addModel()
                .partialState().with(HalfWindowBlock.FACING, Direction.SOUTH).with(HalfWindowBlock.TYPE, WindowShape.BOTTOM).with(HalfWindowBlock.ARCH,true)
                .modelForState().modelFile(window_half_bottom).rotationY(180).addModel()
                .partialState().with(HalfWindowBlock.FACING, Direction.SOUTH).with(HalfWindowBlock.TYPE, WindowShape.FULL_BLOCK).with(HalfWindowBlock.ARCH,false)
                .modelForState().modelFile(window_half).rotationY(180).addModel()
                .partialState().with(HalfWindowBlock.FACING, Direction.SOUTH).with(HalfWindowBlock.TYPE, WindowShape.FULL_BLOCK).with(HalfWindowBlock.ARCH,true)
                .modelForState().modelFile(window_half_arch).rotationY(180).addModel()
                .partialState().with(HalfWindowBlock.FACING, Direction.WEST).with(HalfWindowBlock.TYPE, WindowShape.TOP).with(HalfWindowBlock.ARCH,false)
                .modelForState().modelFile(window_half_top).rotationY(270).addModel()
                .partialState().with(HalfWindowBlock.FACING, Direction.WEST).with(HalfWindowBlock.TYPE, WindowShape.TOP).with(HalfWindowBlock.ARCH,true)
                .modelForState().modelFile(window_half_arch_top).rotationY(270).addModel()
                .partialState().with(HalfWindowBlock.FACING, Direction.WEST).with(HalfWindowBlock.TYPE, WindowShape.MIDDLE).with(HalfWindowBlock.ARCH,false)
                .modelForState().modelFile(window_half_middle).rotationY(270).addModel()
                .partialState().with(HalfWindowBlock.FACING, Direction.WEST).with(HalfWindowBlock.TYPE, WindowShape.MIDDLE).with(HalfWindowBlock.ARCH,true)
                .modelForState().modelFile(window_half_middle).rotationY(270).addModel()
                .partialState().with(HalfWindowBlock.FACING, Direction.WEST).with(HalfWindowBlock.TYPE, WindowShape.BOTTOM).with(HalfWindowBlock.ARCH,false)
                .modelForState().modelFile(window_half_bottom).rotationY(270).addModel()
                .partialState().with(HalfWindowBlock.FACING, Direction.WEST).with(HalfWindowBlock.TYPE, WindowShape.BOTTOM).with(HalfWindowBlock.ARCH,true)
                .modelForState().modelFile(window_half_bottom).rotationY(270).addModel()
                .partialState().with(HalfWindowBlock.FACING, Direction.WEST).with(HalfWindowBlock.TYPE, WindowShape.FULL_BLOCK).with(HalfWindowBlock.ARCH,false)
                .modelForState().modelFile(window_half).rotationY(270).addModel()
                .partialState().with(HalfWindowBlock.FACING, Direction.WEST).with(HalfWindowBlock.TYPE, WindowShape.FULL_BLOCK).with(HalfWindowBlock.ARCH,true)
                .modelForState().modelFile(window_half_arch).rotationY(270).addModel();
    }
